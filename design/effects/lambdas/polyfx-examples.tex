...
$\hosafe{ \forall \phi \subseteq \{ \kwa{File.write, Socket.write} \}. \Unit \rightarrow_{\phi} \Unit}{\kwa{File.*}}$ \\
= $\{ \kwa{File.write, Socket.write} \} \subseteq \{ \kwa{File.*} \} \land \safe{\Unit \rightarrow_{\{\kwa{File.write, Socket.write}\}} \Unit}{\kwa{File.*}}$ \\

\noindent
Both the $\hosafe{\hat \tau_i, \varepsilon_s}$ and $\fx{\hat \tau_i} \subseteq \varepsilon_s$ check fails, because $\{ \kwa{File.write, Socket.write} \} \subseteq \{ \kwa{File.write} \}$ is false. If we were to intersect $\fx{\hat \tau_i}$ with the effects of the capabilities being passed in, we would get a tighter bound on the actual effects of $\hat \tau_i$, and the check would succeed. Similar thing with $\hosafe{\hat \tau_i, \varepsilon_s}$. Recall that $\hosafe{\hat \tau_i, \varepsilon_s}$ is a version of $\hofx{\hat \tau_i} \subseteq \varepsilon_s$ which distributes over the subterms in $\hat \tau_i$. If we intersect the $\hofx{\hat \tau_i}$ with the capabilites passed in, this check would also pass.

How do we define this intersection? Several ways come to mind.

\subsubsection{Extra Argument}

Define a two-variable version of $\kwa{effects}$ which has an extra argument containing all of the imported capabilities. This function will return the effects, but will produce a (potentially) tighter upper-bound using the information from the imported capabilities. This function will be defined the same on non-polymorphic types as the one-variable version, but for polymorphic functions it is defined in the following way:

\begin{itemize}
	\item $\hofx{\hat \tau_i, \overline{\hat \tau}} = \hofx{\hat \tau_i} \cap \bigcup_{i} \fx{\hat \tau_i, \overline{\hat \tau}}$
	\item $\fx{\hat \tau_i, \overline{\hat \tau}} = \fx{\hat \tau_i} \cap \bigcup_{i} \hofx{\hat \tau_i, \overline{\hat \tau}}$
\end{itemize}

Do a similar thing for $\kwa{safe}$ and $\kwa{ho \hyphen safe}$. The premises of \textsc{$\varepsilon$-Import} now use the versions with the extra argument.

\subsubsection{Extra Premises}

Modify the premises of \textsc{$\varepsilon$-Import}. Add a new predicate, $\kwa{is \hyphen poly}(\hat \tau)$ which is true iff $\hat \tau$ is a polymorphic type. The premises are now:

\begin{itemize}
	\item $\neg \kwa{is \hyphen poly}(\hat \tau) \implies \fx{\hat \tau} \subseteq \varepsilon_s$
	\item $\kwa{is \hyphen poly}(\hat \tau) \implies \fx{\hat \tau} \cap \bigcup_{i} \fx{\hat \tau_i} \subseteq \varepsilon_s$
\end{itemize}
...
Define $\varepsilon' = \bigcup_i \fx{\hat \tau_i}$, the sum of the effects in scope. Then define $\kwa{replace}(\lambda \phi \subseteq \varepsilon. \hat \tau) = \lambda \phi \subseteq \varepsilon'. \hat \tau$. Then for any polymorphic capability $\hat \tau_i$< instead of doing e.g. $\kwa{effects}(\hat \tau)$, do $\kwa{effects}(\kwa{replace}(\hat \tau, \varepsilon'))$. (How would this work for polymorphic types?)
