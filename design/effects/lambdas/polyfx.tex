...
\subsection*{Defintion ($\kwa{effects :: \hat \tau \times \overline{\hat \tau} \rightarrow \varepsilon}$)}

\begin{enumerate}
	\item $\kwa{effects}(\hat \tau, \overline{\hat \tau}) = \kwa{effects}(\hat \tau) \cap \bigcup_{i} \kwa{effects}(\hat \tau_i)$, if $\hat \tau$ is polymorphic.
	\item $\kwa{effects}(\hat \tau, \_) = \fx{\hat \tau}$, otherwise.
\end{enumerate}

\noindent
\textbf{Note: the definitions given for non-polymorphic types could also be refined to give a more precise upper-bound on the effects they capture. This definition is also probably deficient when there is another polymorphic function in scope.}
...
\subsection*{Definition ($\kwa{ho \hyphen effects :: \hat \tau \times \overline{\hat \tau} \rightarrow \varepsilon}$)}

\begin{enumerate}
	\item $\kwa{ho \hyphen effects}(\hat \tau, \overline{\hat \tau}) = \kwa{ho \hyphen effects}(\hat \tau) \cap \bigcup_{i} \kwa{ho \hyphen effects}(\hat \tau_i)$, if $\hat \tau$ is polymorphic.
	\item $\kwa{ho \hyphen effects}(\hat \tau, \_) = \hofx{\hat \tau}$, otherwise.
\end{enumerate}
...
\infer[\textsc{($\varepsilon$-Subsume)}]
	...
	{\hat \Gamma \vdash e: \hat \tau ~\kw{with} \varepsilon & \hat \Gamma \vdash \hat \tau <: \hat \tau' & \varepsilon \subseteq \varepsilon'}\\[2ex]
