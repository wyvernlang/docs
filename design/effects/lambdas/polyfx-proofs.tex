...
\fbox{\textsc{$\varepsilon$-PolyFxAbs}.} Then $\hat e$ has the form given in (1). Suppose $\hosafe{\hat \tau}{\varepsilon}$. By inversion on $\kwa{ho \hyphen safe}$ we know (2-3). By inversion on \textsc{$\varepsilon$-PolyFxAbs}, we also know (4). By definition of $\kwa{effects}$, we have (5).
...
Because of the convention of $\alpha$-converting variables, $\Phi$ is fresh, so at the point where a binding for $\Phi$ is introduced, $\Phi$ will be a never-before-seen variable name. Therefore, WLOG, we can assume no binding for $\Phi$ occurs in $\hat \Gamma$. Knowing this, we can apply the narrowing lemma in reverse to obtain (8). By widening the bound on $\Phi$, we get (9). Then by an application of \textsc{S-PolyFxAbs}, we get the theorem conclusion.
