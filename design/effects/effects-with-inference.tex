...
{If $\Gamma \vdash e : \tau$, then one of the following is true:
\begin{itemize}
	\item $\Gamma \vdash e : \{ \bar d \}~\kw{with} \varepsilon$
	\item $\tau = \{ \bar d \}$ and $\Gamma \vdash e : \{ \bar d~\kw{captures} \varepsilon_C \}~\kw{with} \varnothing$, for some $\varepsilon_C$.
\end{itemize}}
{By induction the typing judgement.

\proofcase{\textsc{T-Var}, \textsc{T-Resource}, \textsc{T-New$_\sigma$}}{
	Use \textsc{$\varepsilon$-Var}, \textsc{$\varepsilon$-Resource}, \textsc{$\varepsilon$-New$_\sigma$} respectively.

}

\proofcase{\textsc{T-OperCall}}{
We have $e = e_1.\pi$ and $\Gamma \vdash e_1 : \{ \bar r \}$. By inductive assumption, $\Gamma \vdash e_1 : \{ \bar r \}~\kw{with} \varepsilon$. Applying \textsc{$\varepsilon$-OperCall}, we get $\Gamma \vdash e_1.\pi : \{ \bar r \}~\kw{with} \varepsilon$.
}

\proofcase{\textsc{T-New$_d$}}{
Select $\Gamma' = \Gamma$. Then $\Gamma', x : \{ \bar d~\kw{captures} \varepsilon_C \} \vdash \overline{d = e}~\kwa{OK}$, because we know $\Gamma, x : \{ \bar d \} \vdash \overline{d = e}~\kwa{OK}$. By an application of \textsc{C-NewObj}, we get $\Gamma \vdash e : \{ \bar d~\kw{captures} \varepsilon_C \}~\kw{with} \varnothing$, for $\varepsilon_C = \kwa{effects}(\Gamma)$.
}

\proofcase{\textsc{T-MethCall$_\sigma$}}{
Then $e = e_1.m_i(e_2)$. From inversion on the rule we know $\Gamma \vdash e_1 : \{ \bar \sigma \}$ and $\Gamma \vdash e_2 : \tau_2$. By inductive assumption we know $\Gamma \vdash e_1 : \{ \bar \sigma \}~\kw{with} \varepsilon_1$. If $\tau_2 = \{ \bar d \}$, then applying the inductive hypothesis gives us $\Gamma \vdash e_2 :  \tau_2~\kw{with} \{ \bar d~\kw{captures} \varepsilon_C \}$. \textbf{However, the types are different and because there's no sub-typing we run into a problem here. If we knew $\{ \bar d~\kw{captures} \varepsilon_C \} <: \{ \bar d \}$ then we'd be fine.}
}
}
...
\begin{itemize}
	\item $e$ has the form $\newd$; and $\Gamma \vdash \hat e : \overline{d_i ~\kw{with}\varepsilon_i = e_i}$, where 
