...
\begin{itemize}
	...
	\item $e$ is an expression, and $e \longrightarrow e'~|~\varepsilon$, and $\Gamma \vdash \hat e : \type{\hat \tau}{\hat \varepsilon}$, where $\hat tau = \tau$ and $\varepsilon \subseteq \hat \varepsilon$.
\end{itemize}
...
We also know $e = \newsig$ and $\Gamma \vdash \sigma_i = e_i~\kw{OK}$. By applying the definition of $\kwa{label}$, define $\hat e = \kwa{label}(\newsig) = \kwa{new}_\sigma~x \Rightarrow \overline{\sigma = \kwa{label}(e)}$. To type $\hat e$ we want to use \textsc{$\varepsilon$-NewObj}; to do that we need to know $\Gamma, x : \{ \bar \sigma \} \vdash \overline{ \sigma = e }$.\\
