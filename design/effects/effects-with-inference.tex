...
\begin{itemize}
	...
	\item $\keywadj{label \hyphen helper}(\sigma = e) = \sigma = \keywadj{label}(e, \Gamma)$
	\item $\keywadj{label \hyphen helper}(\keywadj{def}~m(y : \tau_2) : \tau_3 = e) = \keywadj{def}~m(y : \tau_2) : \tau_3~\keyw{with} \keywadj{effects}(\Gamma \cap \keywadj{freevars}(e)) = \keywadj{label}(e, \Gamma)$
\end{itemize}
...
\proofcase{$e = \newsig$}{
By definition, $\keywadj{label}([e'/z](\newsig), \Gamma) = \keywadj{label}(\keywadj{new}_\sigma~x \Rightarrow \overline{\sigma = [e'/z]e})$, which is $\keywadj{new}_\sigma~x \Rightarrow \keywadj{label \hyphen helper}(\overline{\sigma = [e'/z]e})$. Take any particular $\sigma_i = [e'/z]e_i$. Then $\keywadj{label \hyphen helper}(\sigma_i = [e'/z]e_i)$ is defined as $\sigma_i = \keywadj{label}([e'/z]e_i)$. By inductive assumption, this is the same as $\sigma_i = [\keywadj{label}(e')/z](\keywadj{label}(e_i))$. Therefore, $\keywadj{new}_\sigma~x \Rightarrow \keywadj{label \hyphen helper}(\overline{\sigma = [e'/z]e})$ is the same as $\keywadj{new}_\sigma~x \Rightarrow \overline{\sigma = [\keywadj{label}(e')/z]e}$. By ``factoring out'' the substitution, this is the same as $[\keywadj{label}(e')/z](\newsig)$
}
}
...
\end{itemize}
\noindent
Applying definitions, $\keywadj{label}(v_1.m_i(v_2)) = (\keywadj{label}(v_1)).m_i(\keywadj{label}(v_2)) = \hat v_1.m_i(\hat v_2)$. Before labeling, the object $v_1$ has method $m_i$ with body $e_{body}$. The labeled version, $\hat v_1$, has method $m_i$ with body $\keywadj{label}(\Gamma, e_{body}) = \hat e_{body}$. Because $\hat v_1$ and $\hat v_2$ are values, we can apply the reduction \textsc{E-MethCall3$_\sigma$} to $\hat v_1.m_i(\hat v_2)$; therefore $\hat v_1.m_i(\hat v_2) \longrightarrow [\hat v_1 / x, \hat v_2 / y]\hat e_{body}~|~\varnothing$. Lastly, because of the linearity of $\keywadj{label}$ over $\keywadj{substitution}$, $\keywadj{label}(e_B) = \keywadj{label}([v_1/x, v_2/y]e_{body}) = [\keywadj{label}(v_1)/x, \keywadj{label}(v_2)/y](\keywadj{label}(e_{body})))$, which is $[\hat v_1/x, \hat v_2/y] \hat e_{body} = \hat e_B$ by assumptions.\\
}


\subcase{ \textsc{E-MethCall3$_d$} \textbf{TODO}
}
