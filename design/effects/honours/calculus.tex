...
Firstly, suppose $\hat e_i$ has type $\hat \tau_i$. In general, if we wanted to approximate what effects $\hat e_i$ might incur when executed, we might produce a summary $\varepsilon$ of its effects and try to replace the annotations on $\hat \tau_i$ with $\varepsilon$. The approximation would be type safe if $\hat \tau_i <: \annot{\erase{\hat \tau}}{\varepsilon}$ and $\fx{\hat \tau_i} \cup \hofx{\hat \tau_i} \subseteq \varepsilon$. However, replacing all annotations with $\varepsilon$ is not usually a type safe thing to do: functions are contravariant, so the subtyping relation recursively flips on the input-types. These input-types endow $\hat \tau$ with its higher-order effects, so the subsetting relation between $\varepsilon$ and the (constituent) higher-order effects of $\hat \tau_i$ would also keep flipping. However, if $\varepsilon = \fx{\hat \tau} = \hofx{\hat \tau}$, the contravariant behaviour would not matter. In \textsc{$\varepsilon$-Import}, we know $\fx{\hat \tau} \subseteq \varepsilon_s$. Furthermore, $\hosafe{\hat \tau_i}{\varepsilon_s}$ implies $\varepsilon_s \subseteq \hofx{\hat \tau_i}$. Therefore, the approximation of $\hat \tau_i$ obtained by replacing its annotations with $\varepsilon_s$ will correctly summarise the effects of $\hat e_i$. This result is given as a pair of lemmas below.
...
Secondly, if \textsc{E-Import2} is applied, the reduction has the form $\import{\varepsilon_{s}}{x}{\hat v_i}{e} \longrightarrow [\hat v_i/x]\annot{e}{\varepsilon_s}~|~\varnothing$. If $\hat \Gamma \vdash \hat v_i: \hat \tau_i~\kw{with} \varepsilon_i$ and $x: \erase{\hat \tau} \vdash e: \tau$, it is reasonable to assume that $\hat \Gamma \vdash \annot{e}{\varepsilon_s}: \annot{\tau}{\varepsilon_s}~\kw{with} \varepsilon_s$, because $\varepsilon_s$ is an upper-bound on all the effects that can be incurred by $e$ and it correctly summarises the  correctly summarises what the capabilities in $\hat \tau$ can do.

To show it is safe requires a few things. First, if $\hat \Gamma \vdash \import{\varepsilon_s}{x}{\hat v_i}{e}: \annot{\tau}{\varepsilon_s}~\kw{with} \varepsilon_s$, the reduced expression must type in the same context, so we need $\hat \Gamma \vdash [\hat v_i/x]\annot{e}{\varepsilon_s}: \hat \tau_B~\kw{with} \varepsilon_B$. The reduction incurs no effects, so $\varepsilon = \varnothing$. Since $e$ cannot exceed the authority in $\varepsilon_{s}$ the approximation $\varepsilon_B = \varepsilon_{s} = \varepsilon_A$ would be safe. To be type-safe we need $\tau_B <: \tau_A$. What is $\tau_B$? Intuitively, $e_B = [\hat v_i/x]\annot{e}{\varepsilon_s}$, so the type of $\annot{e}{\epsilon_s}$ ought to be $\annot{\tau}{\varepsilon}$. Then, by the substitution lemma, we get $\hat \Gamma \vdash [\hat v/x]\annot{e}{\varepsilon_s}: \annot{\tau}{\varepsilon_s}~\kw{with} \varepsilon_s$.  

Based on our analysis, we need $\hat \Gamma, x: \hat \tau \vdash \annot{e}{\varepsilon_s}: \annot{\tau}{\varepsilon_s} ~\kw{with} \varepsilon_s$. The existence of this judgement is the subject of the following lemma.
