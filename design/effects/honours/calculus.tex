...
\textit{Case:} \textsc{$\varepsilon$-Import}. Then $\hat e = \import{\varepsilon}{x}{\hat e''}{e}$. If $\hat e''$ is a non-value then $\hat e$ reduces by \textsc{E-Import1}. Otherwise $\hat e$ reduces by \textsc{E-Import2}.
\end{proof}
...
In this subcase the reduction has the form $\import{\varepsilon_{select}}{x}{\hat v}{e} \longrightarrow [\hat v/x]\annot{e}{\varepsilon}~|~\varnothing$. To show it is safe requires a few things. First, if $\hat \Gamma \vdash \import{\varepsilon_{select}}{x}{\hat v}{e}: \hat \tau_A~\kw{with} \varepsilon$, the reduced expression must type in the same context: $\hat \Gamma \vdash [\hat v/x]\annot{e}{\varepsilon}: \hat \tau_B~\kw{with} \varepsilon_B$. The reduction incurs no effects, so $\varepsilon = \varnothing$. Since $e$ cannot exceed the authority in $\varepsilon_{select}$, the approximation $\varepsilon_B = \varepsilon_{select} = \varepsilon_A$ would be safe. To be type-safe we need $\tau_B <: \tau_A$. But what is $\tau_B$? Intuitively, if $x: \erase{\hat \tau} \vdash e: \tau$, and the expression after reduction is $[\hat v/x]\annot{e}{\varepsilon}$, then the type after reduction ought to be $\annot{\tau}{\varepsilon}$.

Based on our analysis, we expect $\hat \Gamma, x: \hat \tau \vdash \annot{e}{\varepsilon}: \annot{\tau}{\varepsilon}~\kw{with} \varepsilon$. Then, by the substitution lemma, we get $\hat \Gamma \vdash [\hat v/x]\annot{e}{\varepsilon}: \annot{\tau}{\varepsilon}~\kw{with} \varepsilon$. This first judgement is the subject of the following annotation lemma.
...
\begin{enumerate}
	...
	\item $\kwa{effects}(\hat \tau) \cup \hofx{\annot{\tau}{\varepsilon}} \subseteq \varepsilon_{select}$
	\item $\kwa{ho \hyphen safe}(\hat \tau, \varepsilon)$
\end{enumerate}

\noindent
Let $\varepsilon' = \varepsilon_{select}~\cup~ \fx{\annot{\Gamma}{\varnothing}}$. Then $\hat \Gamma, \kwa{annot}(\Gamma, \varepsilon'), y: \hat \tau \vdash \kwa{annot}(e, \varepsilon') : \kwa{annot}(\tau, \varepsilon')~\kw{with} \varepsilon'$.
\end{lemma}

The premises of the annotation lemma are very specific to the premises of \textsc{$\varepsilon$-Import}, but generalised to accommodate a proof by induction: $e$ might typecheck in a $\Gamma$ with ambient bindings: $\Gamma, y: \kwa{erase}(\hat \tau) \vdash e: \tau$. Then $e$ can use any resources bound in $\Gamma$, so whatever effects they capture $\fx{annot{\Gamma}{\varnothing}}$ are added to $\varepsilon_{select}$ to give a a safe approximation $\varepsilon'$.
