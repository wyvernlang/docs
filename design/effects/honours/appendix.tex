...
\begin{lemma}[$\epscalc$ Safety Lemma 1]
$\safe{\hat \tau}{\varepsilon}$ implies $\fx{\hat \tau} \subseteq \varepsilon$.
\end{lemma}

\begin{lemma}[$\epscalc$ Safety Lemma 2]
$\hosafe{\hat \tau}{\varepsilon}$ implies $\hofx{\hat \tau} \subseteq \varepsilon$.
\end{lemma}

\begin{proof}

If $\hat \tau = \{ \bar r \}$ then $\hofx{\hat \tau} = \varnothing$, so Lemma 2 is vacuously true; if $\safe{\{ \bar r \}}$ then $\{ r.\pi \mid r \in \bar r, \pi \in \pi \} = \fx{\{ \bar r\}} \subseteq \varepsilon$, so Lemma 1 is true. Otherwise $\hat \tau = \hat \tau_1 \rightarrow_{\varepsilon'} \hat \tau_2$. Proceed by induction on derivations of $\kwa{safe}$ and $\kwa{ho \hyphen safe}$.
