...
\textbf{
	\begin{itemize}
	\item $\kwa{effects}(\Gamma)$ is an upper-bound on $\hat \varepsilon$, but we omit this from the proof (for now) to keep it simple.
	\item In the second case of the theorem statement we can use Invariance of Runtime Under $\kwa{label}$ and soundness of fully-annotated programs; to apply that to $e$ we need to know $e$ can be typed with an effect.
	\end{itemize}
}
}
...
There are three reduction rules which could be applied to a method call $e_1.m_i(e_2)$ on an unlabeled object.\\
	...
	\subcase{\textsc{E-MethCall1}. Then we know $e_1 \longrightarrow e_1'~|~\varepsilon$ and $e_1.m_i(e_2) \longrightarrow e_1'.m_i(e_2)~|~\varepsilon$. By definition, $\hat e = \kwa{label}(e_1.m_i(e_2)) = (\kwa{label}(e_1)).m_i(\kwa{label}(e_2)) = \hat e_1.m_i(\hat e_2)$.\\
	
\noindent
Because $\Gamma \vdash e_1 : \{ \bar d \}$ and $\Gamma \vdash e_2 : \tau_2$, by applying the inductive assumption to each we learn $\Gamma \vdash \hat e_1 : \type{\{ \bar \sigma \}}{\hat \varepsilon_1}$ and $\Gamma \vdash \hat e_2 : \tau_2~\kw{with} \hat \varepsilon_2$. \textbf{At this point we need to justify that first inductive assumption on $e_1$. We need to know $\hat e_1$ still has the method $m_i$ as part of its type (and that the new label $\varepsilon_i$ is actually effect-safe)}	 \\

\noindent
\textbf{We would want to apply \textsc{$\varepsilon$-MethCall}, giving us $\Gamma \vdash \hat e_1.m_i(\hat e_2) : \type{\tau_3}{\hat \varepsilon_1 \cup \hat \varepsilon_2 \cup \varepsilon_3 }$, where $\varepsilon_3$ is the label on $m_i$. This would be $\kwa{effects}(\Gamma) \cap e_i$, where $e_i$ is the body of $m_i$, so this type has the effect-set $\hat \varepsilon_1 \cup \hat \varepsilon_2 \cup (\kwa{effects}(\Gamma) \cap e_i)$. $\varepsilon \subseteq \hat \varepsilon_1$ is an inductive assumption, so this type contains the runtime effects.}
	}
	
	
	\subcase{\textsc{E-MethCall3$_d$}. Then we know $e_1 = v_1 = \newd$ and $e_2 = v_2$ and $v_1.m_i(v_2) \longrightarrow [v_1/x, v_2/y]e~|~\varnothing$.\\
	}
...
\begin{itemize}
	\item $\Gamma \vdash \hat e : \hat \tau~\keyw{with} \hat \varepsilon$, where $\hat \varepsilon \subseteq \varepsilon$ and $\tau = \hat \tau$
		\item $e$ has the form $\newd$ and $\Gamma \vdash \hat e : \overline{d_i ~\kw{with}\varepsilon_i = e_i}$, where $\varepsilon_i = \kw{effects}(\Gamma) \cap e_i$
\end{itemize}
...
\end{itemize}
\textbf{
Similar reasoning above. Need to use inductive case on $e_1$, but to achieve that must first flesh out the exact theorem statement (base case more important: see \textsc{C-NewObj} below)}\\
}
...
\noindent
For each $i$, $d_i = e_i~\keywadj{OK}$ only matches \textsc{$\varepsilon$-ValidImpl$_d$}. By inversion on that rule, $\Gamma, y : \tau_2 \vdash e: \tau_3$ and $d_i = \keyw{def} m(y : \tau_2) : \tau_3~\keyw{with} \varepsilon_3$. Applying definitions, $\hat e = \keywadj{label}(\newsig) = \keywadj{new}_d~x \Rightarrow \keywadj{label \hyphen helper}(\overline{d = e})$. Then for each $i$, $\keywadj{label \hyphen helper}(\keywadj{def}~m(y : \tau_2) : \tau_3 = e) = \keywadj{def}~m(y : \tau_2) : \tau_3~\keyw{with}  \keywadj{effects}(\Gamma \cap \keywadj{freevars}(e_i)) = \keywadj{label}(e_i)$. Let $\hat e_i = \keywadj{label}(e_i)$. \\

\noindent
\textbf{To type the labeled object need to know $\Gamma \vdash \sigma_i = e_i~\kwa{OK}$ in order to apply \textsc{$\varepsilon$-NewObj}, where $\sigma_i$ is the signature on method $m_i$ after labeling. We only know $\Gamma \vdash d_i = e_i~\kwa{OK}$.}
}
}
