...
\paragraph{}
Note the distinction between methods (usually denoted $m$) and operations (usually denoted $\mu$). An operation can only be invoked on a resource. Resources can only have operations invoked on them. A method can only be invoked on an object. Objects can only have methods invoked on them.
...
~~~~~~~~~~
\infer[\textsc{(T-Let)}]
	{\Gamma \vdash \keyw{let} x = e_1~\keyw{in} e_2 : \tau_2}
	{\Gamma \vdash e_1 : \tau_1 & \Gamma, x : \tau_1 \vdash e_2 : \tau_2}
~~~~~~~~~~
\infer[\textsc{(T-$\lambda$)}]
	{\Gamma \vdash \lambda x:\tau_1 . e~:~ \tau_1 \rightarrow \tau_2}
	{\Gamma, x : \tau_1 \vdash e : \tau_2}\\[5ex]
~~~~~~~~~~
\infer[\textsc{(T-Apply$\lambda$)}]
	{\Gamma \vdash f(x) : \tau_2}
	{\Gamma \vdash f : \tau_1 \rightarrow \tau_2 & \Gamma \vdash x : \tau_1}
\end{array}
\]
