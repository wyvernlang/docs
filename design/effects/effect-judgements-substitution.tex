...
\noindent \textbf{Notes:}
\begin{itemize}
	\item A 'big-step' involves \textit{zero} or more applications of a small-step.
	\item Big-step rules accumulate the run-time effects produced by the individual small-steps.
...
{Work in the mini calculus for fully-annotated programs. If $\Gamma \vdash e_A : \tau_A~\keyw{with} \varepsilon_A$ and $e \longrightarrow e_B~|~\varepsilon$, then $\varepsilon \subseteq \varepsilon_A$. \\\\}
...
\thm{Theorem 5.6. (Big-Step Soundness For Mini Calculus)}
{Work in the mini calculus for fully-annotated programs. If $\Gamma \vdash e_A : \tau_A~\keyw{with} \varepsilon_A$ and $e \longrightarrow_* e_B~|~\varepsilon$ then $\varepsilon \subseteq e_A$.\\\\}
{If the big-step involves zero steps then the theorem is vacuously satisfied. Otherwise the big-step involves more than one step. Induct on the number of steps. \\
...
\begin{adjustwidth}{1.5em}{0pt}
	\fbox{Inductive Case.}~ If there is a big-step of length n+1 then by \textsc{E-BigStep3} it can be decomposed into a big-step of length n; $e_1 \longrightarrow_* e_n~|~\varepsilon_n$; and a big-step of length 1; $e_n \longrightarrow_* e_{n+1}~|~\varepsilon$. Then $e_1 \longrightarrow_* e_{n+1}~|~\varepsilon_n \cup \varepsilon$ is the entire big-step. \\
	
	\noindent
	Let the type derivations be $\Gamma \vdash e_1 : \tau_1~\keyw{with} \varepsilon_1$ and $\Gamma \vdash e_{n}~\keyw{with} \varepsilon_{n}$ \\
	
	\noindent
	 By inductive assumption on the smaller big-steps, we have that $\varepsilon \subseteq \varepsilon_n$ and $\varepsilon_n \subseteq \varepsilon_1$. Then $\varepsilon_1 \cup \varepsilon_n \subseteq \varepsilon_1$. $\varepsilon_1$ is the static effect information at the start of the big-step, so we're done.
\end{adjustwidth}
}

\thm{Theorem 5.6. (Extension)}
{If the following are true:
	...
	\end{itemize}
Then $\exists \varepsilon_i \mid \varepsilon \subseteq \varepsilon_i \subseteq \keywadj{effects}(\Gamma)$. Letting $\sigma_i = d_i~\keyw{with} \varepsilon_i$, then also $\Gamma \vdash (\keyw{new} x \Rightarrow \sigma_i = e_i) : \{ \sigma_i = e_i \}~\keyw{with} \varnothing$.\\

\noindent
\fbox{Note.} We only consider an object with a single method $m_i$. The result holds for objects with multiple methods by extending each method $m_i$ individually to $m_i'$ as per this theorem, then using them to collectively build a new annotated object $v_1'$ containing the methods $m_i'$.\\\\
}
{Hiya \\}

\thm{Theorem 5.7. (Soundness)}
{If $\Gamma \vdash e_A : \tau~\keyw{with} \varepsilon$ and $e_A \rightarrow e_B~|~\varepsilon_B$, then $\varepsilon_B \subseteq \varepsilon$. \\\\}
{ }
...
\thm{Theorem 5.6. (Soundness Of Effect Typing).}
