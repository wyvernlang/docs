...
\begin{theorem}[Authority Safety] An object can gain authority only in the following four cases:
\begin{itemize}
\item (Potentially) Long-term addition, i.e. the authority will be preserved in the store:
\begin{enumerate}
\item When an object's field is assigned a new principal. (On the object field assignment if the object does not have (long-term) authority over the new field value.)
\end{enumerate}
\item Short-term addition, i.e. the authority is reflected on stack only and will be gained only during the program execution:
\begin{enumerate}\addtocounter{enumi}{1}
\item When a principal, over which the object does not have authority, is passed in as an argument to an object's method.
\item When executing in an object's method, a new principal is created.
\item When executing in an object's method, a method call returns a principal over which the object does not have authority.
\end{enumerate}
\end{itemize}
\end{theorem}
...
The program begins with a creation of an object and an empty stack. There are three actions that can be done to the object: field access (E-Field), field assignment (E-Assign), and method call (E-Meth). Let us consider how each of them changes the authority of the object. (This can be generalized to objects within a program.)

Case E-Field: $l.f~|~\mu \rightarrow l_1~|~\mu$. From the premise, we get $l \mapsto \{ x \Rightarrow \overline{d} \}_{s} \in \mu$ and $\keyw{var} f : \tau = l_1 \in \overline{d}$.
...
\end{align*}
Thus there is no change to authority and the theorem holds.
...
&= pointsto(l_1, \mu') \cup pointsto(\overline{d}, \mu') & (\textsc{auth-stack-nocall})
\end{align*}
By \textsc{pointsto-principal} and \textsc{pointsto-data}, $pointsto(l_1, \mu) = pointsto(l_1, \mu')$.

\noindent However, $pointsto(\overline{d}, \mu') / pointsto(\overline{d}, \mu) \subseteq pointsto(l_2, \mu')$.

Thus if $l_2$ is a pure object, the authority of $l_1$ does not increase; if $l_2$ is a principal, $l_1$ gains authority over $l_2$ (in accordance with case 1 of the theorem). In both cases, the theorem holds.

Case E-Meth: $l_1.m(l_2)~|~\mu \rightarrow l_1.m(l_2) \rhd [l_2/y][l_1/x]e~|~\mu$. From the premise, we get $l_1 \mapsto \{ x \Rightarrow \overline{d} \}_{s} \in \mu$ and $\keyw{def} m(y : \tau_1) : \tau_2 = e \in \overline{d}$.

\noindent Let us consider $l_1$'s authority in the two states. In the initial state, we have:
\begin{align*}
auth(l_1, l_1.m(l_2), \mu) &= auth_{store}(l_1, \mu) \cup auth_{stack}(l_1, l_1.m(l_2), \mu) & (\textsc{auth-config})\\
&= pointsto(l, \mu) \cup pointsto(\overline{d}, \mu) \cup auth_{stack}(l, l_1.m(l_2), \mu) & (\textsc{auth-store})\\
&= pointsto(l, \mu) \cup pointsto(\overline{d}, \mu) & (\textsc{auth-stack-nocall})
\end{align*}
In the final state, we have:

\noindent$auth(l, l_1.m(l_2) \rhd [l_2/y][l_1/x]e, \mu)$
\vspace{-7pt}
\begin{align*}
...
Here the authority of the object increases by $pointsto(l_2, \mu) \cup pointsto(e, \mu) \cup auth_{stack}(l_1, [l_2/y][l_1/x]e, \mu)$, which depends on the concrete values of $l_2$ and $e$. If $l_2$ is a principal, the authority of $l_1$ increases by at least $\{ l_2 \}$, which is in accordance with case 2 of the theorem. The rest of the authority increase depends on $e$ and would be a short-term, runtime-only authority increase. Let us now consider what expressions $e$ can contain and how those expressions change the authority of $l_1$.
...
Thus the caller object $l_1$ gained authority over the newly created principal in accordance with case 3 of the theorem.
...
Thus there was no gain in $l_1$'s authority, and the theorem holds.
...
\noindent\textit{Case $l_3$ is a principal:} Updating the authority in the initial state:
...
\noindent\textit{Case $l_3$ is pure:} Updating the authority in the initial state:
...
%Then we have $l_1.m(l_2) \rhd E'[l_1.f]~|~\mu \longrightarrow l_1.m(l_2) \rhd E'[l_3]~|~\mu$, where $E'$ does not contain any method call stack frames. From the premise, we get $l_1 \mapsto \{ x \Rightarrow \overline{d} \}_{s} \in \mu$ and $\keyw{var} f : \tau = l_3 \in \overline{d}$.
%
%\noindent Let us consider $l_1$'s authority in the two states. In the initial state, we have:
%
%\noindent$auth(l_1, l_1.m(l_2) \rhd E'[l_1.f], \mu)$
%\vspace{-7pt}
%\begin{align*}
%&= pointsto(l_1, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) \cup pointsto(l_1.f, \mu) & (Lemma~4)\\
%&= pointsto(l_1, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) & (\textsc{pointsto-otherexp})
%\end{align*}
%In the final state, we have:
%
%\noindent$auth(l_1, l_1.m(l_2) \rhd E'[l_3], \mu)$
%\vspace{-7pt}
%\begin{align*}
%&= pointsto(l_1, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) \cup pointsto(l_3, \mu) & (Lemma~4)
%\end{align*}
%As $\keyw{var} f : \tau = l_3 \in \overline{d}$, by \textsc{pointsto-decls} and \textsc{pointsto-varl}, $pointsto(l_3, \mu) \subset pointsto(\overline{d}, \mu)$ and
%\[
%auth(l_1, l_1.m(l_2) \rhd E'[l_3], \mu) = pointsto(l_1, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu)
%\]
%Thus $l_1$'s authority does not increase, and the theorem holds.

Subcase E-Assign:
Since fields are private, it must be $l_1$ accessing the field, and this subcase is equivalent to Case E-Assign above.
%Then we have $l_1.m(l_2) \rhd E'[l_1.f = l_3]~|~\mu \longrightarrow l_1.m(l_2) \rhd E'[l_3]~|~\mu'$, where $E'$ does not contain any method call stack frames. From the premise, we get $l_1 \mapsto \{ x \Rightarrow \overline{d} \}_{s} \in \mu$, $\keyw{var} f : \tau = l_4 \in \overline{d}$, $\overline{d}' = [\keyw{var} f : \tau = l_3/\keyw{var} f:\tau = l_4]\overline{d}$, and $\mu' = [l_1 \mapsto \{ x \Rightarrow \overline{d}' \}_{s}/l_1 \mapsto \{ x \Rightarrow \overline{d} \}_{s}]\mu$. 
%
%\noindent Let us consider $l_1$'s authority in the two states. In the initial state, we have:
%
%\noindent$auth(l_1, l_1.m(l_2) \rhd E'[l_1.f = l_3], \mu)$
%\vspace{-7pt}
%\begin{align*}
%&= pointsto(l_1, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) \cup pointsto(l_1.f = l_3, \mu) & (Lemma~4)\\
%&= pointsto(l_1, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) \cup pointsto(l_3, \mu)& (\textsc{pointsto-otherexp})
%\end{align*}
%In the final state, we have:
%
%\noindent$auth(l_1, l_1.m(l_2) \rhd E'[l_3], \mu')$
...
%&= pointsto(l_1, \mu') \cup pointsto(\overline{d}, \mu') \cup pointsto(E', \mu') \cup pointsto(l_3, \mu') & (Lemma~4)
%\end{align*}
%Since we are considering small-step semantics, within this evaluation step, no other assignment could have taken place. Therefore, there were no changes to $l_1$, $E'$, or $l_3$, and $pointsto(l_1, \mu) = pointsto(l_1, \mu')$, $pointsto(E', \mu) = pointsto(E', \mu')$, and $pointsto(l_3, \mu) = pointsto(l_3, \mu')$. However, $pointsto(\overline{d}, \mu') / pointsto(\overline{d}, \mu) \subseteq pointsto(l_3, \mu')$.
%
%Thus if $l_3$ is a pure object, the authority of $l_1$ does not increase; if $l_3$ is a principal, $l_1$ gains authority over $l_3$ (in accordance with case 1 of the theorem). In both cases, the theorem holds.
...
Thus if $l_3$ is a pure object or $l_3$ is a principal and $l_5$ is a pure object, $l_1$'s authority does not increase (the latter is by \textsc{pointsto-data}). If $l_3$ and $l_5$ are both principals, $l_1$ gains authority over $l_5$ (in accordance with case 4 of the theorem). In all the cases, the theorem holds.
