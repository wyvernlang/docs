...
\infer[\textsc{(T-StackFrames)}]
	...
	{\Gamma~|~\Sigma \vdash l_1 : \{ \overline{\sigma_e} \}_s & \keyw{def}~ m : \tau_2 \rightarrow \tau_1 \in \overline{\sigma_e} & \Gamma~|~\Sigma \vdash l_2 : \tau_2 & \Gamma,~l_2 : \tau_2~|~\Sigma \vdash e : \tau_1} \\[5ex]
...
\]
By the induction hypothesis, $\Gamma~|~\Sigma \vdash [e'/z]e_1 : \{ \overline{\sigma_e} \}_s$ and $\Gamma~|~\Sigma \vdash [e'/z]e_2 : \tau_2$. Then, by \textsc{T-Meth}, $\Gamma~|~\Sigma \vdash [e'/z]e_1.m([e'/z]e_2) : \tau_1$, i.e. $\Gamma~|~\Sigma \vdash [e'/z](e_1.m(e_2)) : \tau_1$.
\\
\pcase[\textsc{T-Field}]
...
\[
\Gamma,~z : \tau'~|~\Sigma \vdash e_1 : \{ \overline{\sigma_i} \}_s~~~~~\keyw{var}~ f : \tau \in \overline{\sigma_i}~~~~~\Gamma,~z : \tau'~|~\Sigma \vdash e_2 : \tau
\]
By the induction hypothesis, $\Gamma~|~\Sigma \vdash [e'/z]e_1 : \{ \overline{\sigma_i} \}_s$ and $\Gamma~|~\Sigma \vdash [e'/z]e_2 : \tau$. Then, by \textsc{T-Assign}, $\Gamma~|~\Sigma \vdash  [e'/z]e_1.f = [e'/z]e_2 : \tau$, i.e. $\Gamma~|~\Sigma \vdash [e'/z](e_1.f=e_2) : \tau$.
\\
\pcase[\textsc{T-Bind}]
...
\[
\Gamma,~z : \tau'~|~\Sigma \vdash l_1 : \{ \overline{\sigma_e} \}_s~~~~~\keyw{def}~ m : \tau_2 \rightarrow \tau_1 \in \overline{\sigma_e}~~~~~\Gamma,~z : \tau'~|~\Sigma \vdash l_2 : \tau_2~~~~~\Gamma,~l_2 : \tau_2,~z : \tau'~|~\Sigma \vdash e : \tau_1
\]
Locations are not affected by the substitution, and by the induction hypothesis, $\Gamma,~l_2 : \tau_2~|~\Sigma \vdash [e'/z]e : \tau_1$. Then, by \textsc{T-StackFrame}, $\Gamma~|~\Sigma \vdash l_1.m(l_2) \rhd [e'/z]e : \tau_1$, i.e. $\Gamma~|~\Sigma \vdash [e'/z](l_1.m(l_2) \rhd e) : \tau_1$.
\\
\pcase[\textsc{T-Sub}]
$e = e$, and from the premise, we get $\Gamma,~z : \tau'~|~\Sigma \vdash e : \tau_1$ and $\tau_1 <: \tau_2$. By the induction hypothesis, $\Gamma~|~\Sigma \vdash [e'/z]e : \tau_1$ and $\tau_1 <: \tau_2$. Then, by \textsc{T-Sub}, $\Gamma~|~\Sigma \vdash [e'/z]e : \tau_2$.
\\
\pcase[\textsc{T-Decls}]
From the premise, we get $\forall j,~d_j \in \overline{d},~\sigma_{ij} \in \overline{\sigma_i},~\Gamma, z : \tau'~|~\Sigma \vdash_{s} d_j : \sigma_{ij}$. By the induction hypothesis, $\forall j,~d_j \in \overline{d},~\sigma_{ij} \in \overline{\sigma_i},~\Gamma~|~\Sigma \vdash_{s} [e'/z]d_j : \sigma_{ij}$. Then, by \textsc{T-Decls}, $\Gamma~|~\Sigma \vdash_{s} [e'/z]\overline{d} : \overline{\sigma_{i}}$.
\\
