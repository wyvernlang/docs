...
\[
    [l/x](\keyw{bind} y = e_1~\keyw{in} e_2) = 
\begin{cases}
    \keyw{bind} y = [l/x]e_1~\keyw{in} e_2, & \text{if } x \not= y\\
    \keyw{bind} y = [l/x]e_1~\keyw{in} e_2, & \text{if } x = y
\end{cases}
\]
...
\infer[\textsc{(DT-DefPure)}]
  {\Gamma~|~\Sigma \vdash_{\keywadj{pure}} \keyw{def} m(x : \tau_1) : \tau_2 = e; d~:~\keyw{def} m : \tau_1 \rightarrow \tau_2; \sigma}
  {\def\arraystretch{1.6}
  \begin{array}{c}
\Gamma_{stateful} = \{x : \{ \sigma \}_{\keywadj{stateful}} ~|~ x : \{ \sigma \}_{\keywadj{stateful}} \in \Gamma\} \\
\Gamma_{pure} = \Gamma \setminus \Gamma_{stateful}~~~~~~~\Gamma_{pure},~x : \tau_1~|~\Sigma \vdash e : \tau_2~~~~~~~\Gamma~|~\Sigma \vdash_{\keywadj{pure}} d : \sigma
  \end{array}}\\[5ex]
...
\\
\pcase[\textsc{T-Bind}]
$e = \keyw{bind} x = e_1~\keyw{in} e_2 : \tau_2$, and although there are two subcases to consider, depending on whether $x$ is $z$ or another variable, both subcases yield the same results: If $x = z$, then $[e'/z](\keyw{bind} x = e_1~\keyw{in} e_2) = \keyw{bind} x = [e'/z]e_1~\keyw{in} e_2$, and if $x \not= z$, then $[e'/z](\keyw{bind} x = e_1~\keyw{in} e_2) = \keyw{bind} x = [e'/z]e_1~\keyw{in} e_2$. From the premise, we get $\Gamma,~z : \tau'~|~\Sigma \vdash e_1 : \tau_1$, and by the induction hypothesis, $\Gamma~|~\Sigma \vdash [e'/z]e_1 : \tau_1$. Then, by \textsc{T-Bind}, $\Gamma~|~\Sigma \vdash \keyw{bind} x = [e'/z]e_1~\keyw{in} e_2 : \tau_2$, i.e. $\Gamma~|~\Sigma \vdash [e'/z](\keyw{bind} x = e_1~\keyw{in} e_2) : \tau_2$.
\\
\pcase[\textsc{T-Loc}]
