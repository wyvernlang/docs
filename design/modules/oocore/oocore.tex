...
\infer[\textsc{(pointsto-emptydecl)}]
  {pointsto(\epsilon; d,\mu) = pointsto(d,\mu)}
  {}\\[5ex]
...
%\item In the \textsc{auth-stack} rule, $l''$ was intended, and $l'$ can be arbitrary.  It's likely that $l$ is part of $e'$, since it got substituted for the method argument in $e'$. But perhaps the argument was not used, or it was used but is not being used any more, in which case, $l$ no longer has authority to $l'$. As for the premise, it is just making sure that as we go down the stack, we do not miss any calls to methods of $l$.
%
%To see why, try applying the rule to a stack that has two calls, and try to make it match the inner rather than the outer one. This is possible without the premise, but impossible with it.
...
\item The \emph{authority of an object or an expression} is the aggregated authority of all reachable objects. (Topological definition.)

\item The authority of an object can change due to actions performed by other objects. For example:
\begin{itemize}
\item If a principle A, authority for which an object B does not have, is passed in as an argument to the object B's method by some caller object C, which has authority over A, then B (perhaps only temporarily) gains authority over A, or
\item If an object A, for which another object B has authority, creates a new object C, A gains authority over C via B.
\end{itemize}

\item The authority of an expression can change as the expression is being evaluated and executed and variables are substituted with concrete values (i.e., objects).

\item The authority of an object may also be determined by the behavior of the code stored in the object (methods' bodies, $auth_{store}$).
...
\begin{itemize}
  ...
  \item $\{ l' \} \not\in pointsto(l, \mu)$,
  \item $pointsto(l', \mu') \in auth_{stack}(l'', e, \mu)$, \textbf{\emph{~~~[The caller $l''$ previously had the authority that $l$ gained]}}
  \item $auth(l, e', \mu') = auth(l, e, \mu) \cup pointsto(l',\mu')$.\\
  \end{enumerate}
  \item \textbf{Method return \emph{[INVALID]}:}
  \begin{enumerate}
  \item $e = E[l.m(l') \rhd l'']$, $e' = E[l'']$,
  \item $\{ l'' \} \not\in pointsto(l, \mu)$,
%  \item \textbf{\emph{TODO: The caller previously had the authority that $l$ gained}}, and
  \item $auth(l, e', \mu') = auth(l, e, \mu) \cup pointsto(l'',\mu')$.\\
  \end{enumerate}
\end{itemize}
