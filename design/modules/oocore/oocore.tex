...
\begin{align*}auth(l_D, e', \mu') &= auth_{store}(l_D, \mu') \cup auth_{stack}(l_D, e', \mu') & (\textsc{auth-config})\\
 ...
 &= \{ l_D \} \cup pointsto(l_E, \mu') & (\textsc{pointsto-varx, -emptydecl})\\
...
\begin{align*}auth(l_D, e, \mu) &= auth_{store}(l_D, \mu) \cup auth_{stack}(l_D, e, \mu) & (\textsc{auth-config})\\
 ...
 &= \{ l_D \} \cup pointsto(\keyw{var} x_e : Int = 5, \mu) & (\textsc{pointsto-varx, -emptydecl})\\
 &= \{ l_D \} \cup pointsto(5, \mu) & (\textsc{pointsto-new})\\
...
\begin{itemize}
\item $l_F$ gains authority over $l_G$? temporarily? statically via $auth_{store}$?
\end{itemize}
