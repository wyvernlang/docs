...
\infer[\textsc{(pointsto-call-principal)}]
  ...
  {l \mapsto \{ x \Rightarrow d \}_\keywadj{stateful} \in \mu & \keyw{def} m(y : \tau_1) : \tau_2 = e \in d}\\[5ex]
  
\infer[\textsc{(pointsto-call-data)}]
  {pointsto(l.m(l') \rhd e,\mu) = pointsto(e,\mu)}
  {l \mapsto \{ x \Rightarrow d \}_\keywadj{pure} \in \mu & \keyw{def} m(y : \tau_1) : \tau_2 = e \in d}\\[5ex]
...
\begin{theorem}[Authority Safety -- Version 2] If
\begin{enumerate}
\item $\mu : \Sigma$,
...
\begin{enumerate}
  \item \textbf{Object creation:} $e = \keywadj{new}_s(x \Rightarrow d)$, $e' = l'$,\\
  $l' \not\in dom(\mu)$, $l' \mapsto \{ x \Rightarrow d \}_s$, and\\
  $auth(l, e', \mu') = auth(l, e, \mu) \cup pointsto(l', \mu')$.
  \item \textbf{Method call:} $e = l.m(l')$, $e' = l.m(l') \rhd e''$,\\
  $\{ l' \} \not\in pointsto(l, \mu)$,\\
  and $auth(l, e', \mu') = auth(l, e, \mu) \cup pointsto(l',\mu')$.
  \item \textbf{Method return:} $e = l.m(l') \rhd e''$, $e' = l''$,\\
  $\{ l'' \} \not\in pointsto(l, \mu)$,\\
  and $auth(l, e', \mu') = auth(l, e, \mu) \cup pointsto(l'',\mu')$.
\end{enumerate}
...
\begin{theorem}[Authority Safety -- Version 1] If $\mu : \Sigma$, $\varnothing~|~\Sigma \vdash e : \tau$, and $(e, \mu) \stepsto (e', \mu')$, then $\forall l \in dom(\mu)$, if $auth(l, e', \mu') \supset auth(l, e, \mu)$, then one of the following holds:

\begin{enumerate}
  \item \emph{\textbf{Creation:}} $e = E[\keywadj{new}_s(x \Rightarrow d)]$, $e' = E[l']$, and $auth(l, e', \mu') = auth(l, e, \mu) \cup \{ l' \}$.  \emph{Check that the creator was $l$.}
  \item \emph{\textbf{Call:}} $e = E[l.m(l')]$, $e' = E[l.m(l') \rhd e]$, and $auth(l, e', \mu') = auth(l, e, \mu) \cup pointsto(l',\mu)$  \emph{Check that the caller previously had the authority that $l$ gained.}
  \item \emph{\textbf{Return:}}
\end{enumerate}
