...
\\
\pcase[\textsc{E-Field}]
$e = E[l'.f]$, $e' = E[l'']$, and $\mu' = \mu$. From the premise, we get $l' \mapsto \{ x \Rightarrow d' \}_{s} \in \mu$ and $\keyw{var} f : \tau = l'' \in d'$. Without loss of generality, let $lookupCaller(E) = l$, such that $l \mapsto \{ x \Rightarrow d \}_{s} \in \mu$. By \textsc{lookupCaller}, in a broader context, the following evaluation step is being made: \mbox{$l.m(l''') \rhd E'[l'.f]~|~\mu \longrightarrow l.m(l''') \rhd E'[l'']~|~\mu'$}, where there are no method calls in $E'$. Since we are concerned only with \emph{direct} access (see definition in section~\ref{sec:definitions}), this is the \emph{only} relevant evaluation context.\\
...
$e = E[l'.m(l'') \rhd l''']$, $e' = E[l''']$, and $\mu' = \mu$. Without loss of generality, let $lookupCaller(E) = l$, such that $l \mapsto \{ x \Rightarrow d \}_{s} \in \mu$. By \textsc{lookupCaller}, in a broader context, the following evaluation step is being made: \mbox{$l.m'(l_0) \rhd E'[l'.m(l'') \rhd l''']~|~\mu \longrightarrow l.m'(l_0) \rhd E'[l''']~|~\mu'$}, where there are no method calls in $E'$. Since we are concerned only with \emph{direct} access (see definition in section~\ref{sec:definitions}), this is the \emph{only} relevant evaluation context.\\
...
\pcase[\textsc{E-Meth}]
$e = E[l'.m(l'')]$, $e' = E[l'.m(l'') \rhd [l''/y] [l'/x] e'']$, and $\mu' = \mu$. From the premise, we get $l' \mapsto \{ x \Rightarrow d' \}_{s} \in \mu$ and $\keyw{def} m(y : \tau_1) : \tau_2 = e'' \in d'$. Without loss of generality, let $lookupCaller(E) = l$, such that $l \mapsto \{ x \Rightarrow d \}_{s} \in \mu$. By \textsc{lookupCaller}, in a broader context, the following evaluation step is being made: \mbox{$l.m'(l_0) \rhd E'[l'.m(l'')]~|~\mu \longrightarrow l.m(l_0) \rhd E'[l'.m(l'') \rhd [l''/y] [l'/x] e'']~|~\mu'$}, where there are no method calls in $E'$. Since we are concerned only with \emph{direct} access (see definition in section~\ref{sec:definitions}), this is the \emph{only} relevant evaluation context.\\
...
Since we are considering small-step semantics, within this evaluation step, there were no changes to $l$, $d$, $E'$, and $l'$, and thus $pointsto(l, \mu) = pointsto(l, \mu')$, $pointsto(d, \mu) = pointsto(d, \mu')$, $pointsto(E', \mu) = pointsto(E', \mu')$, and $pointsto(l', \mu) = pointsto(l', \mu')$.\\
