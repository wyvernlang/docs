...
\begin{array}{lllr}
\mu & :: = & \varnothing & store\\
& | & \mu,~l \mapsto \{ \keywadj{self} \Rightarrow d \}_{s}\\
&&\\
...
\begin{array}{c}
\infer[\textsc{(E-New)}]
  {\keywadj{new}_{s}(x \Rightarrow d)~|~\mu \longrightarrow l~|~\mu, l \mapsto \{ \keywadj{self} \Rightarrow d \}_{s}}
  {l \not\in \mu}\\[5ex]
...
\infer[\textsc{(E-Assign)}]
  {l_1.f = v~|~\mu \longrightarrow l_1.f = [l_1/\keywadj{self}]v~|~\mu'}
  {l_1 \mapsto \{ \keywadj{self} \Rightarrow d_v \}_{s} \in \mu & \keyw{var} f:\tau = v \in d_v}\\[5ex]
