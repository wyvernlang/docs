...
Having considered all the possibilities for $d$, we see that $\Gamma,~x : \{ \sigma \}_{s}~|~\Sigma \vdash [e'/z]d : \sigma$. Then, by \textsc{T-New}, $\Gamma~|~\Sigma \vdash \keywadj{new}_{s}(x \Rightarrow [e'/z]d) : \{ \sigma \}_{s}$, i.e. $\Gamma~|~\Sigma \vdash [e'/z](\keywadj{new}_{s}(x \Rightarrow d)) : \{ \sigma \}_{s}$.
\\
\pcase[\textsc{T-Meth}]
...
$e = e.f=e_1$ and, from the premise, we get
\[
\Gamma,~z : \tau'~|~\Sigma \vdash e : \{\sigma\}_s~~~~~\keyw{var}~ f:\tau_1 \in \sigma~~~~~\Gamma,~z : \tau'~|~\Sigma \vdash e_1 : \tau_1
\]
By the induction hypothesis, $\Gamma~|~\Sigma \vdash [e'/z]e : \{\sigma\}_s$ and $\Gamma~|~\Sigma \vdash [e'/z]e_1 : \tau_1$. Then, by \textsc{T-Assign}, $\Gamma~|~\Sigma \vdash  [e'/z]e.f = [e'/z]e_1 : \tau_1$, i.e. $\Gamma~|~\Sigma \vdash [e'/z](e.f=e_1) : \tau_1$.
\\
\pcase[\textsc{T-Loc}] This case is invalid since we cannot make substitutions in a location.
\\
\pcase[\textsc{T-StackFrame}]
$e = l.m(l_1) \rhd e$ and, from the premise, we get
\[
\Gamma,~z : \tau'~|~\Sigma \vdash l : \{\sigma\}_s~~~~~\keyw{def}~ m : \tau_1 \rightarrow \tau_2 \in \sigma~~~~~\Gamma,~z : \tau'~|~\Sigma \vdash l_1 : \tau_1~~~~~\Gamma,~z : \tau'~|~\Sigma \vdash e : \tau_2
\]
Locations are not affected by the substitution, and by the induction hypothesis, $\Gamma~|~\Sigma \vdash [e'/z]e : \tau_2$. Then, by \textsc{T-StackFrame}, $\Gamma~|~\Sigma \vdash l.m(l_1) \rhd [e'/z]e : \tau_2$, i.e. $\Gamma~|~\Sigma \vdash [e'/z](l.m(l_1) \rhd e) : \tau_2$.
\\
\pcase[\textsc{T-Sub}]
...
\\
\item[]  \textit{Subcase} \textsc{E-Field}\textit{:} In this case, $e$ is a value, i.e. a location $l_1$. Then, from  \textsc{T-Field}, we have $\Gamma~|~\Sigma \vdash e : \{\sigma\}_s$ and $\keyw{var}~ f : \tau \in \sigma$. The store $\mu$ does not change, and since \textsc{T-Store} has been applied throughout, the store is well typed, and thus, \mbox{$\Gamma~|~\Sigma \vdash_s \keyw{var}~ f : \tau = l_1 : \keyw{var}~ f : \tau$}. Then, from the premise of \textsc{DT-Varl}, we know that $\Gamma~|~\Sigma \vdash l_1 : \tau$, and the right-hand side is well typed.
\\
\end{itemize}
...
\\
\pcase[\textsc{T-Meth}]
$e = e_1.m(e_2)$, and by the induction hypothesis, either $e_1$ is a value or else it can make a step of evaluation, and likewise $e_2$. Then, there are two subcases:
\\
\begin{itemize}
\item[]  \textit{Subcase} \textsc{E-Congruence}\textit{:} If $e_1$ can take a step or if $e_1$ is a value and $e_2$ can take a step, then rule \textsc{E-Congruence} applies to $e$, and $e$ can take a step.
\\
\item[]  \textit{Subcase} \textsc{E-Meth}\textit{:} If both $e_1$ and $e_2$ are values, i.e. they are locations $l_1$ and $l_2$ respectively, then from the premise of \textsc{T-Meth}, we have $\Gamma~|~\Sigma \vdash l_1 : \{\sigma\}_s$ and $\keyw{def}~ m : \tau_2 \rightarrow \tau_1 \in \sigma$. Since $e_1$ is a value (a location $l_1$), the object must have been created and allocated in the store (i.e. \textsc{E-New} must have been run). Then, since \textsc{T-Store} has been applied throughout, the store is well typed, $l_1 \mapsto \{ x \Rightarrow d \}_{s} \in \mu$, and $\keyw{def} m(y : \tau_1) : \tau_2 = e \in d$. Therefore, the rule \textsc{E-Meth} applies to $e$, $e$ can take a step, and $\mu' = \mu$.
\\
\end{itemize}
...
\\
\begin{itemize}
\item[]  \textit{Subcase} \textsc{E-Congruence}\textit{:} If $e_1$ can take a step, then rule \textsc{E-Congruence} applies to $e$, and $e$ can take a step.
\\
\item[]  \textit{Subcase} \textsc{E-Field}\textit{:} If $e_1$ is a value, i.e. a location $l$, then from the premise of \textsc{T-Field}, we have $\Gamma~|~\Sigma \vdash l : \{\sigma\}_s$ and $\keyw{var}~ f : \tau \in \sigma
$. Since $e_1$ is a value (a location $l$), the object must have been created and allocated in the store (i.e. \textsc{E-New} must have been run). Then, since \textsc{T-Store} has been applied throughout, the store is well typed, $l \mapsto \{ x \Rightarrow d \}_{s} \in \mu$, and $\keyw{var} f : \tau = l_1 \in d$. Therefore, the rule \textsc{E-Field} applies to $e$, $e$ can take a step, and $\mu' = \mu$.
\\
\end{itemize}
...
\item[]  \textit{Subcase} \textsc{E-Congruence}\textit{:} If $e_1$ can take a step or if $e_1$ is a value and $e_2$ can take a step, then rule \textsc{E-Congruence} applies to $e$, and $e$ can take a step.
\\
\item[]  \textit{Subcase} \textsc{E-Assign}\textit{:} If both $e_1$ and $e_2$ are values, i.e. they are locations $l_1$ and $l_2$ respectively, then from the premise of \textsc{T-Assign}, we have $\Gamma~|~\Sigma \vdash l_1 : \{\sigma\}_s$, $\keyw{var}~ f:\tau \in \sigma$, and $\Gamma~|~\Sigma \vdash l_2 : \tau$. Since $e_1$ is a value (a location $l_1$), the object must have been created and allocated in the store (i.e. \textsc{E-New} must have been run). Then, since \textsc{T-Store} has been applied throughout, the store is well typed, $l_1 \mapsto \{ x \Rightarrow d \}_{s} \in \mu$, and $\keyw{var} f:\tau = l \in d$. A new well-typed store can be created as follows: $\mu' = [l_1 \mapsto \{ x \Rightarrow d' \}_{s}/l_1 \mapsto \{ x \Rightarrow d \}_{s}]\mu$, where $d' = [\keyw{var} f:\tau = l_2/\keyw{var} f:\tau = l]d$. Then, the rule \textsc{E-Assign} applies to $e$, and $e$ can take a step.
\\
\end{itemize}
...
\\
\begin{itemize}
\item[]  \textit{Subcase} \textsc{E-Congruence}\textit{:} If $e_2$ can take a step, then rule \textsc{E-Congruence} applies to $e$, and $e$ can take a step.
\\
\item[]  \textit{Subcase} \textsc{E-StackFrame}\textit{:} If $e_2$ is a value, i.e. a location $l_2$, the rule \textsc{E-StackFrame} applies, and $e$ can take a step.
\\
\end{itemize}
