...
\item In our examples, we will use top level type abbreviations.
\end{itemize}
...
\indent if $\keyw{import} M~\keyw{as} M_1; i'$ then $\keyw{val} M_1 = M; trans(i')$\\
\indent if $\keyw{require} \tau~\keyw{as} M_1; i'$ then $ M_1 : \tau,~trans(i')$\\
\indent if $\keyw{instantiate} M_2(M_1^*)~\keyw{as} M; i'$ then $ \keyw{val} M = M_2(M_1^*); trans(i')$\\
...
%\\\\
\noindent$trans(\keyw{resource} \keyw{module} M : \tau; i^*; d) \equiv$

$\keyw{val} M : getTypes(getRequires(i^*)) \rightarrow \tau = $
