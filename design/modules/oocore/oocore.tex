...
\infer[\textsc{(T-New)}]
	...
	{x : \{ \overline{\sigma_i} \}_{s}~|~\Sigma \vdash_s \overline{d} : \overline{\sigma_i} & \forall~\keyw{def} m : \tau_2 \rightarrow \tau_1 \in \overline{\sigma_i},~\keyw{def} m : \tau_2 \rightarrow \tau_1 \in \overline{\sigma_e}} \\[5ex]
...
$\fbox{$\{ \overline{\sigma_i} \} <: \{ \overline{\sigma_i}' \}$}$
\[
\begin{array}{c}
\infer[\textsc{(S-ReflT)}]
  {\{ \overline{\sigma_i} \} <: \{ \overline{\sigma_i} \}}
  {}%\\[5ex]
~~~~~~~~~~
\infer[\textsc{(S-TransT)}]
  {\{ \overline{\sigma_i} \}_1 <: \{ \overline{\sigma_i} \}_3}
  {\{ \overline{\sigma_i} \}_1 <: \{ \overline{\sigma_i} \}_2 & \{ \overline{\sigma_i} \}_2 <: \{ \overline{\sigma_i} \}_3}\\[5ex]
...
\begin{itemize}
  ...
  \begin{enumerate}
  \item $e = E[l.m(l')]$, $e' = E[l.m(l') \rhd [l'/y][l/x]e'']$,
  \item $pointsto(l',\mu) = \{ l' \} \cup pointsto(\overline{d}', \mu)$, {\emph{~~~~~~~~~~[The argument $l'$ is a principal]}}
  \item $\{ l' \} \not\in auth(l, e, \mu)$,
  \item $auth(l, e', \mu') = auth(l, e, \mu) \cup \{ l' \}$.\\
  \end{enumerate}
  ...
  \item $pointsto(l''',\mu) = \{ l''' \} \cup pointsto(\overline{d}''', \mu)$, {\emph{~~~~~~~~[The returned $l'''$ is a principal]}}
...
\[
auth(l, l.m'(l_0) \rhd E'[l'.m(l'') \rhd l'''], \mu) = pointsto(l, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) \cup pointsto(l', \mu) \cup pointsto(l''', \mu)
\]
...
\begin{align*}
auth(l, l.m'(l_0) \rhd E'[l'.m(l'') \rhd [l''/y] [l'/x] e''], \mu) &= pointsto(l, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) \\
&\cup pointsto(l', \mu) \cup pointsto([l''/y] [l'/x] e'', \mu)
\end{align*}

and by \textsc{pointsto-otherexp},
\begin{align*}
auth(l, l.m'(l_0) \rhd E'[l'.m(l'') \rhd [l''/y] [l'/x] e''], \mu) &= pointsto(l, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) \\
&\cup pointsto(l', \mu) \cup pointsto(l'', \mu) \cup pointsto(e'', \mu)
\end{align*}

\todo{Explain that $pointsto(e'', \mu) \subset pointsto(l', \mu)$.}\\
...
$e = E[l.f = l']$, $e' = E[l']$, and $\mu' = [l \mapsto \{ x \Rightarrow \overline{d}' \}_{s}/l \mapsto \{ x \Rightarrow \overline{d} \}_{s}]\mu$. From the premise, we get $l \mapsto \{ x \Rightarrow \overline{d} \}_{s} \in \mu$, $\keyw{var} f:\tau = l'' \in \overline{d}$, and $\overline{d}' = [\keyw{var} f : \tau = l'/\keyw{var} f:\tau = l'']\overline{d}$. Since object fields are private and thus accessible only within the object, $lookupCaller(E) = l$, , i.e. for this step to execute the caller must be the object whose field is being evaluated. By \textsc{lookupCaller}, in a broader context, the following evaluation step is being made: \mbox{$l.m'(l_0) \rhd E'[[l.f = l']~|~\mu \longrightarrow l.m(l_0) \rhd E'[l']~|~\mu'$}, where there are no method call stack frames in $E'$.\\

Let us consider $l$'s authority for the two states. By Lemma 4, in the initial state, we have:
\[
auth(l, l.m'(l_0) \rhd E'[l.f = l'], \mu) = pointsto(l, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) \cup pointsto(l.f = l', \mu)\]
and by \textsc{pointsto-otherexp},
\[
auth(l, l.m'(l_0) \rhd E'[l.f = l'], \mu) = pointsto(l, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) \cup pointsto(l', \mu)
\]
By Lemma 4, in the final state, we have:
\[
auth(l, l.m'(l_0) \rhd E'[l'], \mu') = pointsto(l, \mu') \cup pointsto(\overline{d}, \mu') \cup pointsto(E', \mu') \cup  pointsto(l', \mu')
...
\todo{Consider that $\mu' \not= \mu$.}\\

Thus, $auth(l, e', \mu') = auth(l, e, \mu)$ in compliance with the theorem.
\\

\pcase[\textsc{E-Bind}]
$e = E[\keyw{bind} x = l'~\keyw{in} e'']$, $e' = E[[l'/x] e'']$, and $\mu' = \mu$. Without loss of generality, let $lookupCaller(E) = l$, such that $l \mapsto \{ x \Rightarrow \overline{d} \}_{s} \in \mu$. By \textsc{lookupCaller}, in a broader context, the following evaluation step is being made: \mbox{$l.m'(l'') \rhd E'[\keyw{bind} x = l'~\keyw{in} e'']~|~\mu \longrightarrow l.m'(l'') \rhd E'[[l'/x] e'']~|~\mu$}, where there are no method call stack frames in $E'$.\\

Let us consider $l$'s authority for the two states. By Lemma 4, in the initial state, we have:
\begin{align*}
auth(l, l.m'(l'') \rhd E'[\keyw{bind} x = l'~\keyw{in} e''], \mu) &= pointsto(l, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) \\
&\cup pointsto(\keyw{bind} x = l'~\keyw{in} e'', \mu)
\end{align*}
and by \textsc{pointsto-otherexp},
\begin{align*}
auth(l, l.m'(l'') \rhd E'[\keyw{bind} x = l'~\keyw{in} e''], \mu) &= pointsto(l, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) \\
&\cup pointsto(l', \mu) \cup pointsto(e'', \mu)
\end{align*}

By Lemma 4, in the final state, we have:
\[
auth(l, l.m'(l'') \rhd E'[[l'/x] e''], \mu) = pointsto(l, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) \cup pointsto([l'/x] e'', \mu)
\]
and by \textsc{pointsto-otherexp},
\[
auth(l, l.m'(l'') \rhd E'[[l'/x] e''], \mu) = pointsto(l, \mu) \cup pointsto(\overline{d}, \mu) \cup pointsto(E', \mu) \cup pointsto(l', \mu) \cup pointsto(e'', \mu)
\]
Thus, $auth(l, e', \mu') = auth(l, e, \mu)$ in compliance with the theorem.
