...
i & ::= & \keyw{import} URI~[\keyw{as} x] \\
  & |   & \keyw{instantiate} URI(x^*)~[\keyw{as} x] \\
  & |   & \keyw{requires} x : URI \\
&&\\
...
\item $\keywadj{instantiate}$ is for specifying the parameter of some parameterized module and for creating a resource module. (It is assumed that modules can only call native code if they are an FFI module.)
\end{itemize}
...
%$\textit{trans}(\keyw{var} f : \tau; \sigma)$ & $\equiv$ & $\keyw{var} f : \tau; trans(\sigma)$ \\
\\
...
%\\
%$\textit{trans}(\keyw{def} m (x : \tau) : \tau = e; d)$ & $\equiv$ & $\keyw{def} m (x : \tau) : \tau = e; trans(d)$ \\
%$\textit{trans}(\keyw{var} f : \tau = x; d)$ & $\equiv$ & $\keyw{var} f : \tau = x; trans(d)$ \\
%$\textit{trans}(\keyw{var} f : \tau = l; d)$ & $\equiv$ & $\keyw{var} f : \tau = l; trans(d)$ \\
\end{tabular}
\end{center}

\noindent$\textit{trans}(i) \equiv$\\
\indent if $i = \keyw{import} M~\keyw{as} M_1; *$ then $\keyw{val} M_1 = M; trans(*)$\\
\indent if $i = \keyw{require} URI~\keyw{as} M_1; *$ then $ M_1 : URI,~trans(*)$\\
\indent if $i = \keyw{instantiate} URI(M_1^*)~\keyw{as} M; *$ then $ \keyw{val} M = URI(M_1^*); trans(*)$\\
%$\textit{trans}(\keyw{import} M~\keyw{as} M_1; *)$ & $\equiv$ & $\keyw{val} M_1 = M; trans(*)$ \\
%$\textit{trans}(\keyw{require} URI~\keyw{as} M_1; *)$ & $\equiv$ & $ M_1 : URI,~trans(*)$ \\
%$\textit{trans}(\keyw{instantiate} URI(M_1^*)~\keyw{as} M; *)$ & $\equiv$ & $ \keyw{val} M = URI(M_1^*); trans(*)$ \\
%\\
...
\noindent$trans(\keyw{module} M : URI; d) \equiv \keyw{val} M : URI = \keyw{bind} \keyw{in} \keywadj{new}(M \Rightarrow d)$
\\\\
\noindent$trans(\keyw{resource} \keyw{module} M : URI; d) \equiv \keyw{val} M : \keyw{Unit} \rightarrow URI = \keyw{fn} \_ : \keyw{Unit} \Rightarrow~\keyw{bind} \keyw{in} \keywadj{new}(M \Rightarrow d)$
\\\\
%\noindent$trans(\keyw{resource} \keyw{module} M : URI; im^*; re^*; in^*; d) \equiv$
%
%$\keyw{val} M : \keyw{re\_types} \rightarrow URI = \keyw{fn} trans(re^*) \Rightarrow~\keyw{bind} trans(im^*); trans(in^*) \keyw{in} \keywadj{new}(M \Rightarrow d)$
%\\\\
\noindent$trans(\keyw{resource} \keyw{module} M : URI; i^*; d) \equiv$

$\keyw{val} M : getTypes(getRequires(i^*)) \rightarrow URI = $

$\keyw{fn} trans(getRequires(i^*)) \Rightarrow~\keyw{bind} trans(getImportsAndInstantiates(i^*) \keyw{in} \keywadj{new}(M \Rightarrow d)$
%\\\\
%\noindent$trans(\keyw{class} ~ c ~ \{\overline{cd}; \overline{d}\}) \equiv \keyw{type}~ c = \tau_i; ~ \keyw{var}~ c:\tau_c = e$ \\
%$~~~~~~~ \textrm{where}$ \\
...
%$~~~~~~~~~~~~~~ \overline{d_{cl}} = \{\overline{\keyw{meth}~ m:\tau = e}\} \cup \{\overline{\keyw{var}~ f:\tau = e}\}$ \\
