...
~~~~~~~~~~
\infer[\textsc{(T-Store)}]
  {\mu, l \mapsto \{ x \Rightarrow d_v \} : \Sigma, l : \{ \sigma \}_s}
...
\begin{array}{c}
\infer[\textsc{(E-New)}]
  {\keywadj{new}_{s}(l)~|~\mu \longrightarrow [l/x]d_v~|~\mu'}
  {l \mapsto \{ x \Rightarrow d_v \}_{s} \in \mu}\\[5ex]
  
\infer[\textsc{(E-Meth)}]
  {l_1.m(l_2)~|~\mu \longrightarrow l_1.m(l_2) \rhd [l_2/y][l_1/x]e~|~\mu'}
  {l_1 \mapsto \{ x \Rightarrow d_v \}_{s} \in \mu & \keyw{def} m(y:\tau):\tau = e \in d_v}\\[5ex]

\infer[\textsc{(E-Field)}]
  {l_1.f~|~\mu \longrightarrow l_2~|~\mu'}
  {l_1 \mapsto \{ x \Rightarrow d_v \}_{s} \in \mu & \keyw{var} f:\tau = l_2 \in d_v}\\[5ex]

\infer[\textsc{(E-Assign)}]
  {l_1.f = e~|~\mu \longrightarrow l_1.f = [l_1/x]e~|~\mu'}
  {l_1 \mapsto \{ x \Rightarrow d_v \}_{s} \in \mu & \keyw{var} f:\tau = e \in d_v}\\[5ex]
...
%$\fbox{$k \rhd e~|~\mu \longrightarrow k' \rhd e'~|~\mu'$}$
%\[
%\begin{array}{c}
%\infer[\textsc{(E-StackFrame)}]
%  {k \rhd l_1.m(l_2)~|~\mu \longrightarrow k;~l_1.m(l_2) \rhd [l_2/x]e~|~\mu'}
%  {l_1 \mapsto \{ x \Rightarrow d_v \}_{s} \in \mu & \keyw{def} m(x:\tau):\tau = e \in d_v}\\[5ex] 
%\end{array}
%\]
%
%
%\[
%\begin{array}{c}    
%\Delta :: = \varnothing~|~\Delta, t\\[5ex]
%
%\infer[\textsc{(T-New)}]
%  {\Delta~|~\Gamma~|~\Sigma \vdash \keyw{new}_s(t \Rightarrow d) : \{t \Rightarrow \sigma\}_s}
%  {\Delta, t~|~\Gamma~|~\Sigma \vdash d : \sigma}\\[5ex] 
%
%\infer[\textsc{(E-New)}]
%  {\keyw{new}_s(t \Rightarrow d)~|~\mu \longrightarrow l~|~\mu'}
%  {d~|~\mu \longrightarrow d_v~|~\mu' & l \mapsto \{d_v\} \in \mu' & d~|~\mu, l \mapsto \{d_v\}}\\[5ex] 
%
%\end{array}
%\]
