...
\begin{pcases}
\pcase[\textsc{T-Var}]
$e = x$ and, from the premise, we get $x : \tau \in (\Gamma,~z : \tau')$. There are two sub-cases to consider, depending on whether $x$ is $z$ or another variable. If $x = z$, then $[e'/z]x = e'$. The required result is then $\Gamma~|~\varnothing \vdash e' : \tau'$, which is among the assumptions of the lemma. Otherwise, $[e'/z]x = x$, and the desired result is immediate.
\\
\pcase[\textsc{T-New}]
