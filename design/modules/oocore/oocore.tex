...
\infer[\textsc{(pointsto-emptydecl)}]
  {pointsto(d; \epsilon,\mu) = pointsto(d,\mu)}
  {}\\[5ex]
...
\item An object can \emph{directly access} a principle if the object either has a reference to the principle or can access the object via pure methods.
...
\begin{itemize}
\item Authority can be possessed by either a principle (a stateful object) or an expression. (See $pointsto()$ above.)

\item Our definition of an \emph{object} comes directly from object-oriented programming and means an encapsulation of state and functionality. Then, having \emph{authority over a principle} means having access to the principle's state and functionality.

\item \emph{Authority} is the upper bound on the set of principles (stateful objects) that a subject (another principle or an expression) can access and act upon.

\item The \emph{authority of an object or an expression} is the union of the set of all principles (stateful objects) the current principle or expression has access to.
...
\item If a principle A, authority for which an object B does not have, is passed in as an argument to the object B's method by some caller object C, which has authority over A, then B (perhaps only temporarily) gains authority over A.
\item If a principle A, authority for which an object B does not have, was returned from a method call that B initiated, then B gains authority over A.
\end{itemize}
...
\item for some principle $l$, $auth(l, e', \mu') \supset auth(l, e, \mu)$,
\end{enumerate}
...
\begin{itemize}
  ...
  \item $pointsto(l',\mu) = \{ l' \}$, \emph{~~~[The argument $l'$ is a principle]}
  ...
  \item $pointsto(l''',\mu) = \{ l''' \}$, \emph{~~~[The returned $l'''$ is a principle]}
  \item $\{ l' \} \in auth_{stack}(l, e, \mu)$,
  \item $\forall l_o : auth_{stack}(l, e, \mu) \subset auth_{stack}(l_o, e, \mu)$, \textbf{\emph{~~~[The caller was $l$, the ``closest'' principle to $e$]}}
  \item $\{ l''' \} \not\in auth_{stack}(l, e, \mu)$,
  \item $auth(l, e', \mu') = auth(l, e, \mu) \cup \{ l''' \}$.\\
  \end{enumerate}
\end{itemize}
...
A principle cannot lose authority.
\end{theorem}

\begin{corollary}
Once a principle is created, it always stays a principle and cannot become a pure object.
\end{corollary}
...
new($l_D$ =>
  var xd : typeOfE = new($l_E$ => var xe : Int = 5))
\end{lstlisting}
...
%    new($l_D$ =>
%        var xd : typeOfE =
%          new($l_E$ => var xe : Int = 5))
%\end{lstlisting}
%\end{minipage}
%\end{tabular}

\begin{itemize}
\item $l_D$ gains authority over $l_E$:
\end{itemize}

\noindent$e = E[\keywadj{new}(l_E \Rightarrow \keyw{var} x_e : Int = 5)]$, $e' = E[l_E]$

\noindent$auth(l_D, e', \mu') = auth(l_D, e, \mu) \cup \{ l_E \}$
\begin{align*}auth(l_D, e', \mu') &= auth_{store}(l_D, \mu') \cup auth_{stack}(l_D, e', \mu') & (\textsc{auth-config})\\
 &= auth_{store}(l_D, \mu') & (\textsc{auth-stack-nocall})\\
 &= pointsto(l_D, \mu') \cup pointsto(\keyw{var} x_d : typeOfE = l_E, \mu') & (\textsc{auth-store})\\
 &= \{ l_D \} \cup pointsto(\keyw{var} x_d : typeOfE = l_E, \mu') & (\textsc{pointsto-principal})\\
 &= \{ l_D \} \cup pointsto(l_E, \mu') & (\textsc{pointsto-varl, -emptydecl})\\
 &= \{ l_D \} \cup \{ l_E \} & (\textsc{pointsto-principal})\end{align*}
\begin{align*}auth(l_D, e, \mu) &= auth_{store}(l_D, \mu) \cup auth_{stack}(l_D, e, \mu) & (\textsc{auth-config})\\
 &= auth_{store}(l_D, \mu) & (\textsc{auth-stack-nocall})\\
 &= pointsto(l_D, \mu) \cup pointsto(\keyw{var} x_d : typeOfE = \keywadj{new}(l_E => ...), \mu) & (\textsc{auth-store})\\
 &= \{ l_D \} \cup pointsto(\keyw{var} x_d : typeOfE = \keywadj{new}(l_E => ...), \mu) & (\textsc{pointsto-principal})\\
 &= \{ l_D \} \cup pointsto(\keywadj{new}(l_E => \keyw{var} x_e : Int = 5), \mu) & (\textsc{pointsto-varx, -emptydecl})\\
 &= \{ l_D \} \cup pointsto(\keyw{var} x_e : Int = 5, \mu) & (\textsc{pointsto-new})\\
 &= \{ l_D \} \cup pointsto(5, \mu) & (\textsc{pointsto-varx, -emptydecl})\\
 &= \{ l_D \} & (\textsc{pointsto-data})\end{align*}
...
\begin{itemize}
\item $l_F$ is stateful
\item $l_F$ gains authority over $l_G$:
\end{itemize}

\noindent$e = E[\keywadj{new}(l_G \Rightarrow \keyw{var} x_g : Int = 3)]$, $e' = E[l_G]$

\noindent$auth(l_F, e', \mu') = auth(l_F, e, \mu) \cup \{ l_G \}$
\begin{align*}auth(l_F, e', \mu') &= auth_{store}(l_F, \mu') \cup auth_{stack}(l_F, e', \mu') & (\textsc{auth-config})\\
 &= auth_{store}(l_F, \mu') & (\textsc{auth-stack-nocall})\\
 &= pointsto(l_F, \mu') \cup pointsto(\keyw{def} m_f : Unit \rightarrow typeOfG = l_G, \mu') & (\textsc{auth-store})\\
 &= \{ l_F \} \cup pointsto(\keyw{def} m_f : Unit \rightarrow typeOfG = l_G, \mu') & (\textsc{pointsto-principal})\\
 &= \{ l_F \} \cup pointsto(l_G, \mu') & (\textsc{pointsto-def, -emptydecl})\\
 &= \{ l_F \} \cup \{ l_G \} & (\textsc{pointsto-principal})\end{align*}
\begin{align*}auth(l_F, e, \mu) &= auth_{store}(l_F, \mu) \cup auth_{stack}(l_F, e, \mu) & (\textsc{auth-config})\\
 &= auth_{store}(l_F, \mu) & (\textsc{auth-stack-nocall})\\
 &= pointsto(l_F, \mu') \cup pointsto(\keyw{def} m_f : Unit \rightarrow typeOfG = ...), \mu') & (\textsc{auth-store})\\
 &= \{ l_F \} \cup pointsto(\keyw{def} m_f : Unit \rightarrow typeOfG = \keywadj{new}(l_G \Rightarrow ...), \mu) & (\textsc{pointsto-principal})\\
 &= \{ l_F \} \cup pointsto(\keywadj{new}(l_G => \keyw{var} x_g : Int = 3), \mu) & (\textsc{pointsto-def, -emptydecl})\\
 &= \{ l_F \} \cup pointsto(\keyw{var} x_g : Int = 3, \mu) & (\textsc{pointsto-new})\\
 &= \{ l_F \} \cup pointsto(3, \mu) & (\textsc{pointsto-varx, -emptydecl})\\
 &= \{ l_F \} & (\textsc{pointsto-data})\end{align*}
...
\begin{itemize}
\item Is module \lstinline{I} a resource?
\item $l_J$ has authority over $l_H$
\item $l_I$ gains authority over $l_H$ because $l_H$ is passed in as an argument into $l_I$'s method?
\end{itemize}
