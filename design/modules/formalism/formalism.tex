...
\infer[\textsc{(T-AbsNoState)}]
  ...
  \begin{array}{c}
\Gamma_{ref} = \{x : \Reftt \tau'~|~ x : \Reftt \tau' \in \Gamma\} \\ % \{ \overline{x : \tau}~|~ x_i \in freeVarsOf(e_2) \land \tau_i = \Reftt \tau'\} \\
\Gamma_{sf} = \{ x : \tau \xrightarrow{x} \tau'~|~x : \tau \xrightarrow{x} \tau' \in \Gamma \} \\ % \{\overline{x : \tau \xrightarrow{s} \tau'}~|~ x_i \in freeVarsOf(e_2)\} \\
