...
\subsection{Path Equality and Type Widened Expressions}
\begin{lstlisting}[mathescape, style=custom_lang]
var a = new {self $\Rightarrow$
             type L : $\top$ .. $\top$
             var i : self.L = a};
var b = new {self $\Rightarrow$
             var j : {self $\Rightarrow$
                      type L :{self $\Rightarrow$ i:$\top$} .. $\top$
	                   var i : self.L} = a};
var c = new {self $\Rightarrow$
             var k : {var j : {self $\Rightarrow$
                               type L :$\bot$ .. $\top$
	                            var i : self.L}} = b
             def m : $\top$ (x : self.k.j.L){
                 var y = new {self $\Rightarrow$ var l : z.k.j.L = x}; y
             }};
c.m(c.k.j.i)
\end{lstlisting}

c.m(c.k.j.i) $\rightarrow$ c.m(b.j.i) $\rightarrow$ c.m(a.i) $\rightarrow$ c.m(a)
\section{Type System}
	\label{s:type_sys}
...
%       & | & v.f = E\\
       & | & E \unlhd T\\
       & | & \texttt{var} \; x = \texttt{new} \; \{\texttt{self} \Rightarrow \overline{D}\}; \; e & eval \; context \\
       & | & \texttt{var} \; x = \texttt{new} \; \{\texttt{self} \Rightarrow \overline{d_v}\}; \; E& \\
&&\\
