...
\end{theorem}
\begin{proof}
Proceed by induction $\Gamma \vdash S <: T$.
...
\inferrule
  ...
  {}
\end{mathpar}
$\Gamma \vdash T <: U$ can be derived in one of the following cases.
\begin{itemize}
...
\inferrule
  ...
  {}
\end{mathpar}
This clearly presents a contradiction, since by our initial assumption,
$T = \{z \Rightarrow \overline{\sigma'}\} \neq p.L$.
\\
...
\\
\\
\item[]  \textit{Subcase 5} (\textsc{S-Bottom}): 
\begin{mathpar}
\inferrule
  {T = \bot}
  {}
...
\inferrule
  ...
  {}
\end{mathpar}
From the derivation of $\Gamma \vdash p.L <: U$, we get the following cases.
...
\inferrule
  ...
  {}
\end{mathpar}
This results in a contradiction from 
$T \neq p.L = \{z \Rightarrow \overline{\sigma}\}$
\\
\item[]  \textit{Subcase 3} (\textsc{S-Select-Upper}):
\begin{mathpar}
\inferrule
  {T = p.L \\
	\Gamma \vdash p \ni \texttt{type} \; L : S' .. U' \\
  ...
  {}
\end{mathpar}
By our existing premise $\Gamma \vdash S <: S'$, and since $p.L$ is 
well-formed in $\Gamma$, we know that $\Gamma \vdash S' <: U$ and it follows 
that $\Gamma \vdash S <: U$.
\\
\end{itemize}
\end{case}
