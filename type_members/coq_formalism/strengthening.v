...
Proof.
  ...
  assert (Hunbound_t : [i1 dots i2] runbound_t t);
    [intros n' Hge Hlt;
     ...
     inversion Hlt;
     auto|].
  simpl; apply cont_upper with (t:=t [i2] ljump_t n).
  eapply H; eauto.
  eapply H0; eauto.
  ...
  simpl; apply cont_equal with (t:=t [i2] ljump_t n).
  eapply H; eauto.
  eapply H0; eauto.
Qed.
...
Proof.
  ...
  (*s select upper1*)
  apply s_upper1 with (t1:=t1 [i2] ljump_t n).
  ...
  (*s select upper2*)
  apply s_upper2 with (t1:=t1 [i2] ljump_t n).
  ...
  (*s select lower1*)
  apply s_lower1 with (t2:=t2 [i2] ljump_t n).
  ...
  (*s select lower2*)
  apply s_lower2 with (t2:=t2 [i2] ljump_t n).
  ...
  (*s select equal3*)
  apply s_equal3 with (t1:=t1 [i2] ljump_t n).
  ...
  (*s select equal4*)
  apply s_equal4 with (t2:=t2 [i2] ljump_t n).
  ...
  assert (Hrewrite1 : ((str (ds1 [i2]ljump_ss n) rts) :: (G3 [i2]ljump_env n) ++ (G4 [i2]ljump_env n) =
                       ((str ds1 rts) :: G3 [i2]ljump_env n) ++ (G4 [i2]ljump_env n)));
    [crush|rewrite Hrewrite1].
  assert (Hrewrite2 : ((str (ds2 [i2]ljump_ss n) rts) :: (G5 [i2]ljump_env n) ++ (G6 [i2]ljump_env n) =
                       ((str ds2 rts) :: G5 [i2]ljump_env n) ++ (G6 [i2]ljump_env n)));
    [crush|rewrite Hrewrite2].
  assert (Hrewrite3 : (c_ (length (G3 ++ G4))) =
                      ((c_ i) [i2] ljump_e n));
    [assert (Hle : i2 <= i);
    ...
    |rewrite <- ljump_length
     ...
     rewrite <- ljump_length
       with
         ...
         (n:=n)
       in Hrewrite1].
  assert (Hrewrite2 : ((str (ss [i2]ljump_ss n) rts) :: (G1 [i2]ljump_env n) ++ (G2 [i2]ljump_env n)) =
                      (((str ss rts) :: G1) [i2]ljump_env n) ++ (G2 [i2]ljump_env n));
    [simpl; auto
    |].
...
Proof.
  ...
  assert (Hunbound_str : [i1 dots i2] runbound_t (str ss rts));
    [eapply typing_range_unbound_exp; eauto|].
  assert (Hunbound_ss : [i1 dots i2] runbound_ss ss);
    [eapply range_unbound_ty_str; eauto|].
