...
Notation "'str' ss 'rts'" := (t_str ss) (at level 40).
...
Fixpoint left_shift_type (n : nat) (t : ty) : ty :=
  ...
  | str ds rts => str (ds lshift_dts n) rts
  end
...
Fixpoint right_shift_type (n : nat) (t : ty) : ty :=
  ...
  | str ds rts => str (ds rshift_dts n) rts
  end
...
Fixpoint raise_ty (t : ty)(i : nat) : ty :=
  ...
  | str ss rts => str (ss raise_ss (S i)) rts
  end
...
Fixpoint right_jump_t (t : ty) (i n : nat) : ty  :=
  ...
  | str ss rts => str (ss [i] rjump_ss n) rts
  end
...
Fixpoint rename_t (t : ty)(n m : nat) : ty :=
  ...
  | str ss rts => str (ss [S n maps_ss S m]) rts
  end
...
Fixpoint subst (n : nat) (e : exp) (t : ty) : ty :=
  ...
  | str ds rts => str ([ (e raise_e 0) /ss S n ] ds) rts
  end
...
| cont_struct : forall Sig G ds d, in_dty d ds ->
                            Sig en G vdash d cont str ds rts
...
| s_arr : forall Sig G1 t1 t1' t2 t2' G2 i, Sig en G2 vdash t2 <; t1 dashv G1 ->
                                     i = length G1 ->
                                     i = length G2 ->
                                     Sig en t1::G1 vdash ([c_ i /t 0] t1') <; ([c_ i /t 0] t2') dashv t2::G2 ->
                                     Sig en G1 vdash (t1 arr t1') <; (t2 arr t2') dashv G2
                                       
| s_upper1 : forall Sig G1 p L t1 t2 G2, Sig en G1 vdash p ni (type L ext t1) ->
                                  Sig en G1 vdash t1 <; t2 dashv G2 ->
                                  Sig en G1 vdash (sel p L) <; t2 dashv G2
                                    
| s_upper2 : forall Sig G1 p L t1 t2 G2, Sig en G2 vdash p ni (type L ext t1) ->
                                  Sig en G2 vdash t1 <; t2 dashv G2 ->
                                  Sig en G1 vdash (sel p L) <; t2 dashv G2
                                    
| s_lower1 : forall Sig G1 t1 p L t2 G2, Sig en G2 vdash p ni (type L sup t2) ->
                                  Sig en G1 vdash t1 <; t2 dashv G2 ->
                                  Sig en G1 vdash t1 <; (sel p L) dashv G2
                                    
| s_lower2 : forall Sig G1 t1 p L t2 G2, Sig en G1 vdash p ni (type L sup t2) ->
                                  Sig en G1 vdash t1 <; t2 dashv G1 ->
                                  Sig en G1 vdash t1 <; (sel p L) dashv G2
...
| s_equal3 : forall Sig G1 t1 p L t2 G2, Sig en G2 vdash p ni (type L eqt t1) ->
                                  Sig en G2 vdash t1 <; t2 dashv G2 ->
                                  Sig en G1 vdash (sel p L) <; t2 dashv G2
                                    
| s_equal4 : forall Sig G1 t1 p L t2 G2, Sig en G1 vdash p ni (type L eqt t2) ->
                                  Sig en G1 vdash t1 <; t2 dashv G1 ->
                                  Sig en G1 vdash t1 <; (sel p L) dashv G2

| s_struct : forall Sig G1 ds1 ds2 G2 i, i = length G1 ->
                                  i = length G2 ->
                                  Sig en (str ds1 rts)::G1 vdash [c_ i /ss 0] ds1 <;;; [c_ i /ss 0] ds2 dashv (str ds2 rts)::G2 ->
                                  Sig en G1 vdash str ds1 rts <; str ds2 rts dashv G2
...
| t_new : forall Sig G ds ss, Sig en (str ss rts)::G vdash ([c_ length G /ds 0] ds) hasType_ds ([c_ length G /ss 0] ss) ->
                       length G unbound_ss ss ->
                       Sig en G vdash new ds hasType str ss rts
...
Definition lt_ctx (Sig : env)(n : nat) := forall t, In t Sig -> lt_t t n.

Inductive lt_env : env -> nat -> Prop :=
| lt_nil : forall n, lt_env nil n
| lt_cons : forall G t n, lt_t t n ->
                     lt_env G n ->
                     lt_env (t::G) (S n).

Hint Constructors lt_env.
...
| wf_str : forall Sig G ss, (length G) unbound_ss ss ->
                     Sig en (str ss rts)::G vdash ([c_ length G /ss 0] ss) wf_ss ->
                     Sig en G vdash (str ss rts) wf_t
...
| wf_new : forall Sig G ds ss, Sig en G vdash new ds hasType str ss rts ->
                        (length G) unbound_ds ds ->
                        Sig en (str ss rts)::G vdash ([c_ length G /ds 0] ds) wf_ds ->
...
| wfst_nil : nil wf_st
| wfst_con : forall Sig ds, Sig en nil vdash str ds rts wf_t ->
                     Sig wf_st ->
                     str ds rts::Sig wf_st
