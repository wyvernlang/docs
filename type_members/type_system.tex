...
\begin{figure}[h]
\hfill \fbox{$\mu; \Sigma \vdash v \leadsto_{f} \sigma_0, ..., \sigma_i$}
\begin{mathpar}
\inferrule
  {\mu(l) = \{z \Rightarrow ..., \texttt{val} \; f:T, ...\}}
  {\mu; \Sigma \vdash l \leadsto_{f} \texttt{val} \; f:T}
  \quad (\textsc {L\textsubscript{$f$}-Loc})
	\and
\inferrule
  {\mu; \Sigma \vdash v \leadsto_{f} \sigma_0, ..., \sigma_i \\
  	\varnothing; \Sigma; \varnothing \vdash T \prec_z \overline{\sigma} \\
  	\texttt{val} \; f:S \in \overline{\sigma}}
  {\mu; \Sigma \vdash v \unlhd T \leadsto_{f} \sigma_0, ..., \sigma_i, (\texttt{val} \; f:S)}
  \quad (\textsc {L\textsubscript{$m$}-Type})
	\and
\inferrule
  {\mu; \Sigma \vdash v \leadsto_{f} \sigma \\
  	\varnothing; \Sigma; \varnothing \vdash T \prec_z \overline{\sigma} \\
