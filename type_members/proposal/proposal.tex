...
\section{Virtual Types}
In object oriented programming we organise data and functions into logical groupings called objects. The intention is that programmers are more easily able to create abstract representations of data and data structures. We call an object's data fields and functions methods, and these collectively can be referred to as the object's members. We classify data such as objects using types, abstract descriptions of data. When it comes to objects, this means a description of the types of data and functions within. \emph{Virtual Types} (or \emph{type members}) are a programming language feature allowing types to be included as members to objects in the same fields and methods normally are. Below is an example of type members using a simple syntax that I will be using throughout this document.
\begin{lstlisting}[mathescape, style=custom_lang]
new {z $\Rightarrow$
    val f : $\top$ = new {z $\Rightarrow$}
...
\end{lstlisting}
We create a new object with three members. A field \verb|f|, a method \verb|m| and a type member \verb|t|. Our type member is initialised to top type $\top$, the set of all objects. Virtual types give us a similar kind of polymorphism to generic type parameters. They allow us to write generic code for classes of objects, rather than objects with specific types. 

\section{Generics with Virtual Types}
...
\section{Formalising Virtual Types in Wyvern}

Unlike type parameters, there does not exist a standard extensible type system like FGJ. This makes research more difficult. There has been work done in virtual types, albeit less than that done for type parameters, and there are relatively few mainstream languages that use virtual types. Scala is one language that does, however Scala virtual types are not sound. There has been much work on developing a sound calculus for virtual types in Scala, however thus far there has only been one sound type system proposed \cite{amin 2014} using a big step semantics. While this is a significant step forward there still does not exist a sound type system featuring a small step semantics in the style of FGJ \cite{fgj}. While soundness is certainly significant, we feel that a small-step semantics is more desirable than a big-step semantics, especially in the context of designing a wider language.
