...
keywords=[1]{new, objtype, type, casetype, val, def, metadata, keyword, of, fn, with},
      ...
      @\dslblue{EXP BOUNDARY `else' BOUNDARY EXP}@
        fn e1, e2 => ~
          case $self
            ...
            False => $e2
def f(x : Bool)
  x.if @\dslgreen{\{}@branch_1@\dslgreen{\} else \{}@branch_2@\dslgreen{\}}@
\end{lstlisting}
\vspace{-8px}
...
val d = Employee ~
  val connection = new ~
    @\dslgreen{mysql://localhost:3306}@
  val username = "user1"
  ... (* Other fields *)
  val getByID(x:Int) : List = stmts({~})
    @\dslyellow{SELECT * FROM Employee WHERE id = \{}x\dslyellow{\}}@
  ... (* Other methods *)
\end{lstlisting}
\vspace{-8px}
...
\begin{tabular}{ l r l l r l l}
  Program             & $\rho$            & ::= &  $\kappa_{\delta};\theta;e$       & \textcd{programs}  & $\rightarrow$ & \textcd{typekws= typedecls> e>}\\
  Type Keyword        & $\kappa_\delta$   & ::= &  $\emptyset$                      & \textcd{typekws}   & $\rightarrow$ & \textcd{$\epsilon$}\\
                      &                   &     &  $\kappa_{\delta};k[e,\tau]$      & \textcd{typekws}   & $\rightarrow$ & \textcd{typekws= NEWLINE> `\textbf{type}'> ID>}\\
  ...
  Type                & $\tau$            & ::= &  $\mathbf{named}[T]$              & \textcd{type}      & $\rightarrow$ & \textcd{ID=} \\
                      &                   &     &  $\mathbf{objtype}[\chi]$         & \textcd{type}      & $\rightarrow$ & \textcd{`\textbf{objtype}'= objdecls>} \\
  ...
  Expression Keyword  & $\kappa$          & ::= &  $\emptyset$                      & \textcd{expkws}    & $\rightarrow$ & \textcd{$\epsilon$}\\
  ...
  Casetype Cases      & $\chi$            & ::= &  $\emptyset$                      & \textcd{casedecls} & $\rightarrow$ & \textcd{$\epsilon$}\\                 
                      &                   &     &  $C[\tau];\omega$                 & \textcd{casedecls} & $\rightarrow$ & \textcd{casedecls= NEWLINE> ID> ($\epsilon$ | `\textbf{of}'> types)}\\
...
Typekw Env & $\Delta_\kappa$ & ::= & $\emptyset$\\
            &                 &  |   & $\Delta_{\kappa};k[i,\tau]$\\
Declkw Env & $\Theta$        & ::= & $\emptyset$\\
            &                 &  |   & $\Theta,T[\delta,\mu,\zeta]$\\
Type       & $\delta$        & ::= & $?$ ~ | ~ $\tau$\\
Metadata   & $\mu$           & ::= & $?$ ~ | ~ $i:\tau$\\
Keywords   & $\zeta$         & ::= & $?$ ~ | ~ $\dot\kappa$\\
Keyword defs& $\dot\kappa$    & ::= & $\emptyset$\\
...
If ~$\vdash_{\Theta_0}\Theta$, $\vdash_{\Theta}\Delta_\kappa$~ and $\emptyset\vdash_{\Theta}i\Leftarrow\tau$ or $\vdash_{\Theta}i\Rightarrow\tau$, then either $i~\texttt{val}$ or $i\mapsto i'$ such that $\emptyset\vdash^{\Theta}_{\Theta}i'\Leftarrow\tau$.
\end{theorem}

\begin{theorem}[External Type Preservation]
If ~$\vdash_{\Theta_0}\Theta$, $\vdash_{\Theta}\Delta_\kappa$, $\vdash_{\Theta}\Gamma$, and $\Gamma\myvdash e\rightsquigarrow i\Leftarrow\tau$ or $\Gamma\myvdash e\rightsquigarrow i\Rightarrow\tau$ then $\Gamma\myvdash i\Leftarrow\tau$.
\end{theorem}

\begin{theorem}[Compilation]
If ~$\rho\sim\Theta\rightsquigarrow i:\tau$~ then $\vdash\Theta$,~$\vdash_{\Theta}\Delta_\kappa$,~$\emptyset\myvdash i\Leftarrow\tau$.
\end{theorem}
