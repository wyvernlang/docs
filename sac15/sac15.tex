...
\newcommand{\myvdash}{\vdash_{\Theta}}
...
\begin{figure}[ht]
  ...
  \begin{array}{ll}
      ...
      \text{Declarations}\\
      \theta~::=~\emptyset      &\\
      \tabularspace\theta;k[\mathbf{ty},e,\tau] & \textcd{syntax}~k~\textcd{with metadata:}\tau~\textcd{=}~e\\
      \tabularspace\theta;k[\mathbf{syn}(\tau),e]    & \textcd{syntax}~k : \tau~\textcd{=}~e\\
      \tabularspace\theta;k[\mathbf{ana},e]          & \textcd{syntax}~k~\textcd{=}~e\\
...
Declaration Context    & $\Theta$  & ::=   & $\emptyset$\\
                &           &   |   & $\Theta;k[\mathbf{ty},i,\tau]$\\
                &           &   |   & $\Theta;k[\mathbf{syn}(\tau),i]$\\
                &           &   |   & $\Theta;k[\mathbf{ana},i]$\\
                &           &   |   & $\Theta,T[\delta,\mu]$\\
                & $\delta$  &  ::=  & $?$ ~ | ~ $\tau$\\
                & $\mu$     &  ::=  & $?$ ~ | ~ $i:\tau$\\
