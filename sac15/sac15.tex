...
As general-purpose programming languages evolve, they inevitably accumulate \emph{syntactic sugar} to capture common idioms more concisely or naturally. In most contemporary languages, syntactic sugar must be built in by the language's designers, so idioms that arise only situationally are often not available. This has motivated substantial research into mechanisms that decentralize control over a language's concrete syntax, to varying degrees. 

Designing a useful syntax extension mechanisms is non-trivial because the designer can no longer verify simply by comprehensive examination that  no parsing ambiguities are possible and that the desugarings available are semantically well-behaved. The mechanism must instead consider several important issues:

\noindent
\textbf{Composability} It cannot simply allow the base language's syntax to  be modified arbitrarily, because separately defined fragments of syntax that are known not to cause parsing ambiguities individually can nevertheless cause ambiguities when they are composed by interfering directly with one another (e.g. extensions adding support for XML and HTML). %To avoid this issue, the mechanism must control  extensions from one another.

\noindent
\textbf{Hygiene} The desugaring logic  must also be constrained to ensure that the meaning of a term cannot change simply because some of the surrounding variables have been renamed (manually or by a refactoring tool). It should also be possible to identify the binding site of a variable, even if there are intervening uses of sugar. These two situations correspond to inadvertent variable capture and shadowing, respectively. 
...
Most prior approaches to syntax extension, discussed in Sec. \ref{related}, have not achieved at least one of these goals. Recent work on \emph{type-specific languages} (TSLs) does achieve these goals, but only in a limited setting: library providers can define new syntax only for introducing values of a type (i.e. \emph{literal forms}), associating it as metadata with the type when it is declared \cite{TSLs}. Local type inference, specified as a bidirectional type system \cite{Pierce:2000:LTI:345099.345100}, controls which syntax extension is invoked to parse delimited pieces of syntax, so extensions are composable and maintain the typing discipline. The semantics also guarantees hygiene, discussed further later. 

Many forms of syntactic sugar can be implemented as TSLs, but there remain others where TSLs do not suffice: 1) only a single TSL can be associated with a type, and only when it is declared, so alternative syntax for the same type, or for a type not under a user's control, cannot be defined; 2) new syntax for non-introductory operations cannot be defined; and 3) types cannot themselves be declared using specialized syntax. In this paper, we supplement TSLs with \emph{typed syntax macros} (TSMs) designed to support these scenarios while maintaining the strong guarantees above.

We will begin by showing how TSMs can be used at the term level in Sec. X. To support the typing discipline, there are two flavors of term-level TSMs: synthetic TSMs can be used anywhere, while analytic TSMs can only be used in positions where the expected type is known,  like TSLs. We then turn to the level of type declarations  in Sec. X. There, TSMs generate not just a type declaration but also the TSL associated with it, allowing TSLs and TSMs to operate in concert to enable interesting usage scenarios, as we discuss. Finally, we give a minimal type-theoretic account of this mechanism by extending the bidirectionally typed elaboration semantics given previously by Omar et al. in Sec. X, leveraging the same underlying hygiene mechanism and internal language and emphasizing the cohesion of these mechanisms. %   mechanism to support efforts to  decentralize control over the concrete syntax of a typed programming language. 
 We conclude by positioning our approach relative to related work in Sec. X. \todo{section refs}
...
The definition of the type keyword \verb|DBSchema| can be referred to figure \ref{typekw-example-1}. The keyword itself is a \verb|TypeParser|, which will parse in a data schema and then generate \todo{finish describing the parser.}
\begin{figure}
\begin{lstlisting}[style=wyvern]
...
\multicolumn{3}{l}{External Term}\\
 $e$~::=~...                              & \\
\tabularspace$\mathbf{lit}[body]$             & (delimited form, Fig. \ref{f-delimited})\\
\tabularspace$\mathbf{ekey}[k,body](e)$       & $e.k$ (delimited form, Fig. \ref{f-delimited})\\
