...
%      \tabularspace C[\tau];\chi                   & C~\textcd{of}~\tau\\
      ...
      i~::=~...
  \end{array}$
\mycaption{Abstract and concrete forms for TSM Wyvern declarations and terms. Metavariable $\tsm$ ranges over TSM names, $T$ over type names, $dform$ over delimited forms, per Figure \ref{f-delimited}, and $body$ over their bodies.}
\label{formal-syntax}
\end{figure}
...
\textsf{Types}\\
      \tau~::=~\mathbf{named}[T] ~|~ \mathbf{arrow}[\tau, \tau] ~|~ \mathbf{objtype}[\omega]\\
      \tabularspace ~|~ \mathbf{casetype}[\chi] ~|~ X ~|~ \mathbf{Tap}(\tau,\tau) ~|~ \mathbf{Tabs}(X.\tau)\\
      \textsf{Translational Types}\\
      \hat{\tau}~::=~... ~|~ \mathbf{spliced}[\tau]\\
      \mathsf{Kinds}\\
      \kappa~::=~ \star  ~|~ \mathbf{Arr}[\kappa,\kappa]
  \end{array}$
\mycaption{Abstract syntax for TSM Wyvern types and kinds.
