...
(syntax extensions -- problems with composition, hygiene)

(in previous work, we introduced TSLs -- discuss example)
...
type ExpParser = objtype
  def parse(ps : ParseStream, exp : Exp) : Result(Exp)
  metadata : HasTSL = new 
    val parser = (* parser generator *)
...
type HTML = casetype 
  ...
  metadata = new : HasTSL
    val parser = ~
      @\dslblue{...}@ (* HTML parser definition *)
...
expkw simpleHTML : HTML = ~
  val parser = ~
    ...
    @\dslblue{start <- text}@
      fn text => "@\dslred{Text(\$}@text@\dslred{)}@"
    @\dslblue{start <- '<'= EXP>}@
      fn e => '@\dslred{\$}@e'
  ...
  val parser = ~
    @\dslblue{start <- pairs}@
      fn pairs =>
        let ty : Type = ~
          @\dslpurple{objtype}@
            @\dslpurple{type Entry = objtype}@
              @\dslpurple{\{}@map(pairs, Nil, 
                fn ((p, lbl, ty), l) => Cons(~, l))
                  @\dslorange{val \$}@lbl @\dslorange{:  \$}@ty
              @\dslpurple{\}}@
            @\dslpurple{val connection : URL}@
            @\dslpurple{val username : String}@
            @\dslpurple{val password : String}@
            @\dslpurple{...}@
            @\dslpurple{\{}@map(pairs, Nil, 
              (fn ((p, lbl, ty), l) => Cons(~, l)))
        ...
        let md : HasTSL = new 
          val parser = ~
            @\dslblue{start <- ("connect to "= EXP>}@
                      @\dslblue{"username:"= EXP>}@
                      @\dslblue{"password:"= EXP>}@
                      @\dslblue{"table:" EXP>)}@
              fn url, un, pw, table => ~
                @\dslred{new}@ 
                  @\dslred{val connection = \$}@url
                  @\dslred{val username = \$}@un
                  @\dslred{val password = \$}@pw
                  @\dslred{...}@
                  @\dslred{\{}@map(pairs, Nil, 
                    (fn ((p, lbl, ty), l) => Cons(~, l)))
