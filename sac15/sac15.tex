...
\hspace{-5px}$\begin{array}{ll}
      ...
      \tabularspace d; \mathbf{tyaptsm}(T, \tsm, body, e)   & \textcd{type}~T~\textcd{=}~\tsm~dform\\
                                                            & ~~~\textcd{metadata }e\\
      \textsf{Types}\\
      \multicolumn{2}{l}{\tau~::=~\mathbf{named}[T] ~|~ \mathbf{arrow}[\tau, \tau] ~|~ \mathbf{objtype}[\omega]}\\
      \multicolumn{2}{l}{\tabularspace ~|~ \mathbf{casetype}[\chi] ~|~ \mathbf{rec}(T::\kappa.\tau) ~|~ \tau\times\tau ~|~ X ~|~ \tau(\tau) ~|~ \lambda X.\tau}\\
      \textsf{Translational Types}\\
      \hat{\tau}~::=~... ~|~ \mathbf{spliced}[\tau]\\
      \mathsf{Kinds}\\
...
\begin{center}
\AXC{$X::\kappa\in\Delta$}
\UIC{$\Delta\vdash_{\Theta} X::\kappa$}
%%
\AXC{$\Delta,X:\star\vdash_{\Theta}\tau::\kappa$}
\UIC{$\Delta\vdash_{\Theta}\mathbf{Tabs}(X.\tau)::\star\rightarrow \kappa$}
%%
\AXC{$\Delta\vdash_{\Theta}\tau_1::K_1\rightarrow \kappa_2$ ~~~~ $\Delta\vdash_{\Theta}\tau_2::\kappa_1$}
\UIC{$\Delta\vdash_{\Theta}\mathbf{Tap}(\tau_1,\tau_2)::\kappa_2$}
%%
\AXC{$T[\tau,\mu]\in\Theta$ ~~~~ $\Delta\vdash_\Theta\tau::\kappa$}
\UIC{$\Delta\vdash_{\Theta} \mathbf{named}[T]::\kappa$}
\noLine
\QIC{}
\DP
\end{center}
...
\begin{center}
\AXC{$\Delta_{out}\vdash_{\Theta}^{\Psi} \tau\rightsquigarrow {\tau'}::\kappa$}
\UIC{$\Delta_{out};\Delta\vdash_{\Theta}^{\Psi}\mathbf{spliced}[\tau]\rightsquigarrow \tau'::\kappa$}
\DP
\end{center}
\end{figure}
