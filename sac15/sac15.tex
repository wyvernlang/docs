...
keyword DBSchema = new
  val parser = ~
    start <- pairs
      fn (pairs) => (typeDecl:TypeDecl, tslParser:HasTSL)
        let typeDecl = ~
          List(Pair("connection", "URL"), ~)
            List(Pair("username", "String"), ~)
              List(Pair("password", "String"), ~)
                ...
                  List(Pair("stmts", Arrow("SQL", "List")), pairs)
        let tslParser = ~
          start <- ">connect to" BOUNDARY URL BOUNDARY>
                    "username" COLUMN username>
                    "password" COLUMN password>
                    "table"    COLUMN table>
            fn url, un, pw, table, pairs => new ~
              val connection = {$url}
              val username = {$un}
              val password = {$pw}
              {
                foreach($pair : $pairs) {
                  "getBy"+$pair.id "(" $x ":" $pair.type.dom ")" : $pair.type.ran = 
                    "SELECT * FROM" $table "WHERE" $pair.id "=" $x
                }
              }
    pairs <- pair delim pairs>
