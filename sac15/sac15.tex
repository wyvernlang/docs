...
type @\typecolor{Type}@ = casetype
  TVar of ID (* type variables *)
  TLam of ID * Type (* unnamed type constructors *)
  TAp of Type * Type (* type parameter application *)
  TRecursive of ID * Type (* unnamed recursive types *)
  Named of ID
...
\begin{array}{l}
      ...
      ~~\Gamma ::= \emptyset ~|~ \Gamma, x : \tau ~|~ \Delta,X::K
\end{array}
\]
...
\begin{figure}[ht]
\begin{center}
  \AXC{$\Gamma\vdash_{\Theta}^{\Psi} T_1::K_{11}\Rightarrow K_{12}$ ~~~~ $\Gamma\vdash_{\Delta}^{\Psi} T_2::K_{11}$}
  \UIC{$\Gamma\vdash_{\Theta}^{\Psi} T_1~T_2 :: K_{12}$}
  \DP
\end{center}
