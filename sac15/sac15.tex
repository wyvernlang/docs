...
Permissions@acm.org.}
\conferenceinfo{SAC 2015}{April 13 - 17 2015, Salamanca, Spain\\
...
Types declarations are \emph{generative}, i.e. declared types are identified nominally (like datatypes in ML, or classes in Java). They can also declare type parameters, e.g. \lstinlinew{List} declares one type parameter. To be more precise, we say that \lstinlinew{List} is a \emph{type constructor}, in that applying \lstinlinew{List} to a type produces a particular list type, e.g. \lstinlinew{List(String)}. Types have \emph{kind} \lstinlinew{*}, while {type constructors} have arrow kind, e.g. \lstinlinew{List} has kind \lstinlinew{* -> *}. For concision, we use the phrase \emph{declared type} for names having any kind. 
We assume that standard types like \lstinlinew{String}, \lstinlinew{List} and \lstinlinew{Option} are ambiently available (i.e. they are declared in the \emph{prelude}). 

Some types are identified structurally, e.g. tuple types like \lstinlinew{HTML * HTML} and function types like \lstinlinew{HTML -> Unit}. Object and case types written anonymously, rather than via a declaration, are also identified structurally. %Only declared types can be recursive.
...
When the semantics encounters any such literal form, the syntax associated with the declared type that the literal is being analyzed against, here \lstinlinew{HTML}, is used to desugar it.
