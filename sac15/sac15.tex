...
\vspace{3px}
\noindent\textbf{\textit{Contributions}} In this paper, we introduce \emph{typed syntax macros (TSMs)}, which supplement TSLs to support these scenarios while maintaining the strong, and we argue crucial, guarantees described above. Our specific contributions are as follows:% A TSM is invoked explicitly but otherwise benefits from the same mechanisms developed for TSLs. 
...
\item In Sec. \ref{theory}, we outline a type-theoretic account of TSMs by extending the bidirectionally typed elaboration semantics for TSLs given previously by Omar et al. \cite{TSLs}, leveraging essentially the same underlying language and hygiene mechanism at the term level and introducing an analagous type-level hygiene mechanism. A more complete type-theoretic treatment is available in an accompanying technical report \cite{sac15tr}.% (emphasizing the cohesion of these mechanisms). %   mechanism to support efforts to  decentralize control over the concrete syntax of a typed programming language. 
\end{enumerate}
...
A TSL is associated with a declared type by setting its metadata to a value of type \verb|HasTSL|, an object type with a field \verb|parser| of type \verb|Parser(Exp)|. Per the declarations in Figure \ref{exp-prelude}, a parser defines a \verb|parse| function that transforms a \verb|ParseStream| based on the body to a \verb|Result(Exp)|, which is either a desugaring, encoded as a value of type \verb|Exp|, or an indication of a parse error. 
When the semantics encounters a literal form, the parser associated with the declared type that the literal is being analyzed against, here \verb|HTML|, is used. 

Rather than writing a \verb|parse| function explicitly, we make use of the fact that \verb|Parser| itself has a TSL associated with it providing a static \emph{grammar-based parser generator}. This allows us to create a \verb|Parser(Exp)| by giving a number of productions, each of which is followed by a Wyvern function taking in the elaborations of each constituent non-terminal and producing the final desugaring of type \verb|Exp|. The non-terminal \verb|start| serves as the top-level non-terminal. 

The type \verb|Exp| encodes the abstract syntax of Wyvern terms and \verb|Type| analagously encodes the abstract syntax of Wyvern types. These are also equipped with TSLs that provide \emph{quasiquotation}  \cite{quine40book,Bawd99a,ScalaMacros2013}: terms of these types  can be written using Wyvern's concrete syntax, extended with an unquote form \verb|$x| that splices in the value of  \verb|x| of the same type.  %We refer to \cite{TSLs} for further details on these two modes of use.

Splicing can be supported by any TSL because a parser can request that some portion of the parse stream be treated as a host language term, type or variable. For example, the TSL for \verb|HTML| uses the delimiters \verb|<{| and \verb|}>| to mean ``splice in the enclosed term  of type \verb|HTML| here''. The cases \verb|Spliced| in \verb|Exp| and \verb|Type| capture spliced portions of the parse stream. This is the basis of the hygiene mechanism in \cite{TSLs}. The parser generator provides the non-terminals \verb|EXP|, \verb|ID| and \verb|TYPE|, which generate these spliced forms internally. For example, \verb|EXP| is used on line 13 of Figure \ref{f-htmltype}. %The hygiene mechanism for TSLs ensures that only spliced terms can refer to variables in the surrounding scope. New variables cannot be introduced into their scope (functions must be used to communicate between TSLs and the host language), so that binding sites are easy to determine. We will return to splicing in Sec. \ref{theory}. 
...
\label{f-simplehtml}
\end{figure}
TSMs are defined using the \verb|syntax| keyword. Figure \ref{f-simplehtml} shows a synthetic TSM, \verb|simpleHTML|, being defined and used. As indicated, valid uses of the TSM must elaborate to a term of type \verb|HTML|. Like defining a TSL, defining a TSM requires defining a parser, i.e. a statically evaluated value of type \verb|Parser(Exp)|. Note that for expository purposes, we include type annotations that are not strictly needed in comments. 

This example TSM defines an alternative layout-sensitive syntax for HTML that is more concise than the conventional one. We again do so using the parser generator associated with \verb|Parser|. In this case, we take advantage of its support for \emph{Adams grammars}, which allow declarative specifications of layout-sensitive grammars using \emph{layout constraints} within productions \cite{Adams:2013:PPI:2429069.2429129}. Here, the suffix \verb|=| indicates that the left-most column (on any line) occupied by the annotated terminal or non-terminal must occur at the same column as the parent  and \verb|>| indicates that it must be further indented. %More detail on Adams grammars and this syntax for HTML can be found in \cite{TSLs}. 

Applying a TSL is similar to function application: the name of the TSL is followed by a delimited form from Figure \ref{f-delimited}, e.g. on line 7 of Figure \ref{f-simplehtml}. The body of the delimited form is parsed and elaborated by the definition of the TSM. %Note that we do not here address namespacing issues, as standard techniques can be used (e.g. using URIs as in Java). 
Notice that we did not need a type annotation on \verb|heading| because \verb|simpleHTML|  is synthetic, i.e. it declares a type.

 On lines 8-11, we again invoke \verb|simpleHTML|, this time using the same forward referenced delimited form introduced in the work on TSLs to avoid syntactic clashes between explicit delimiters and the extended syntax. The only difference here is again the addition of the \verb|simpleHTML| ``keyword''. 

 Synthetic TSMs address problem (i) from Sec. \ref{sec-intro}: defining more than one possible syntax for a type that either has a TSL  already, or a type which a user cannot modify. Compared to the alternative solution of defining a type \verb|HTML2| with a TSL that has the desired syntax along with a function mapping from \verb|HTML2| to \verb|HTML|, TSMs avoid declaration duplication and avoid  the $\mathcal{O}(n)$ runtime overhead. %TSMs, like TSLs, perform their desugaring at compile-time.

Synthetic TSMs can also define syntax for types identified structurally (e.g. pairs), so they also address problem (ii).
...
Synthetic TSMs are inappropriate for expressing idioms that are valid at many types. For example, consider a case type encoding booleans, \verb|Bool|, shown in Figure \ref{if-example}. Explicit case analysis on booleans is often unnecessarily verbose, so we introduce the more idiomatic \verb|if| construct. Rather than having to build this in to the language, however, we can express it as an analytic TSM. These are distinguished from synthetic TSMs by the absence of a type annotation, because the type of an \verb|if| expression is determined by its branches. 
...
\begin{center}
\AXC{\stackanchor{$\vdash_\Theta^\Psi \theta \sim_\Upsilon \Theta'$ ~~~~ $\Upsilon = \mathtt{T}_1 :: \kappa_1, \ldots, \mathtt{T}_i :: \kappa_i, \ldots, \mathtt{T}_n :: \kappa_n$ ~~~~ $s[\textbf{ty}(n, \kappa, \tau_{md}, i_{tsm})] \in \Psi$}{
  $\mathsf{parsestream}(body)=i_{ps}$ ~~~~ $i_{tsm}.parse(i_{ps}) \Downarrow OK((i_{type},i_{md}))$ ~~~~ $i_{type}\uparrow\hat\tau$ ~~~~ $\emptyset;\emptyset\vdash_{\Theta_0\Theta\Theta'}\hat\tau\rightsquigarrow\tau::\kappa_1 \rightarrow \ldots \rightarrow \kappa_n \rightarrow \kappa_i$
  }}
