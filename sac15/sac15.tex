...
\hspace{-5px}$\begin{array}{ll}
      ...
      \tabularspace d;\mathbf{tytsm}(\tsm, \tau, e) & \textcd{syntax}~\tsm::K~\textcd{with metadata:}\tau\,\textcd{=}\,e\\
      ...
      & ~~~\chi\\
      \tabularspace \tau\times\tau & {\tau *\tau}\\
      \tabularspace X     & X\\
      \tabularspace \mathbf{Tap}(\tau;\tau)   & \tau(\tau)\\
      \tabularspace \mathbf{Tabs}(X.\tau)     & \textcd{tyfn}~X\textcd{=>}~\tau\\
      \mathsf{Kinds}\\
      K~::=~ \star                            & *\\
      \tabularspace K\Rightarrow K                      & K~\textcd{->}~K\\ 
%      \text{Object Fields}\\
%      \omega~::=~\emptyset                      \\
...
%      \tabularspace C[\tau];\chi                   & C~\textcd{of}~\tau\\
      \textsf{Translational Types}\\
      \hat{\tau}~::=~...\\
      \tabularspace \mathbf{spliced}[\tau]\\
      ...
      i~::=~...
  \end{array}$
\mycaption{Abstract and concrete forms for TSM Wyvern. Metavariable $\tsm$ ranges over TSM names, $T$ over type names, $l$ over member labels, $C$ over case names, $x$ over term variables, $X$ over type variables, $dform$ over delimited forms, per Figure \ref{f-delimited}, and $body$ over their bodies.}
\label{formal-syntax}
\end{figure}
...
%%
\AXC{$\Delta,X:\star\vdash_{\Theta}\tau::K$}
\UIC{$\Delta\vdash_{\Theta}\mathbf{Tabs}(X.\tau)::\star\Rightarrow K$}
%%
...
\begin{figure}
\flyingbox{$\Delta;\Delta\vdash_{\Theta}\hat{\tau}\rightsquigarrow \tau::K$}
\begin{center}
\AXC{$\Delta_{out}\vdash_{\Theta}^{\Psi} \tau\rightsquigarrow {\tau'}::K$}
\UIC{$\Delta_{out};\Delta\vdash_{\Theta}^{\Psi}\mathbf{spliced}[\tau]\rightsquigarrow \tau'::K$}
\DP
\end{center}
\end{figure}
