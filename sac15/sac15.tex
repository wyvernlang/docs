...
%%% the next rule
\AXC{\stackanchor{$\Theta_{0}\subset\Theta$ ~~~~ $\vdash_{\Theta} \Psi$ ~~~~ $\nexists~s.(s=s_0\land s[\mathbf{ty}(\_,\_,\_)]\in\Psi)$}{$\vdash_{\Theta}\tau_{md}::\star$ ~~~~ $\emptyset;\emptyset\vdash_{\Theta}i \Leftarrow \mathtt{Parser}(\mathtt{Type}\times\tau_{md})$}}
