...
\begin{center}
\AXC{$\emptyset\vdash_{\Theta_{0}}^{\emptyset}\theta\sim(\Psi;\Theta)$ ~~~~ $\emptyset|\emptyset\vdash_{\Theta_0\Theta}^{\Psi}e\rightsquigarrow i\Rightarrow \tau$}      \RightLabel{(compile)}
...
\AXC{
	...
	{$\Delta\vdash_{\Theta_{0}\Theta_{names}}^{\Psi_{names}}\theta \sim_{defs}(\Psi_{defs};\Theta_{defs})$ ~~~~ $\Delta\vdash^{\Psi_{defs}}_{\Theta_{0}\Theta_{defs}}\theta \sim_{exts}(\Psi;\Theta) $}
} \RightLabel{(rec-decls)}
\UIC{$\Delta\vdash_{\Theta_{0}}^{\Psi} \theta \sim (\Psi;\Theta)$} 
\DP
\end{center}
...
%%% the next rule
\AXC{$T[\tau,\mu]\in\Theta$ ~~~~ $\Delta\vdash_\Theta\tau::K$}
