...
\end{theorem}
\begin{proof}
Base on the proof of core Wyvern external terms, we only need to present the proofs for the new cases extended in external Wyvern:
\begin{itemize}
\item $\mathbf{etsmap}[k,body]$ \todo{finish the proof of these three cases}
\item $\mathbf{etermtsmdef}[k]$
\item $\mathbf{etypetsmdef}[k]$
\end{itemize}
