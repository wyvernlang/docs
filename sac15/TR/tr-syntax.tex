...
\hspace{-5px}$\begin{array}{rrlll}
      ...
      \text{Declarations}	&	d &::=&\emptyset\\
                          &&    | & {d}; \mathbf{syntsm}(s,\tau,e) & \textcd{syntax}~s : \tau~\textcd{=}~e\\
                          &&    | & {d}; \mathbf{anatsm}(s,e) & \textcd{syntax}~s~\textcd{=}~e\\
                          &&    | & {d}; \mathbf{tytsm}(s,\kappa,\tau,e) & \textcd{syntax}~s::\kappa~\textcd{=}\,e\\
                          &&    | & {d};\mathbf{mrectydecl}(\theta_1,...,\theta_n) & \theta_1~\textcd{and}~...~\textcd{and}~\theta_n\\
      \text{Type Declaration}&\theta  &::=&\mathbf{tydecl}(\T,\tau,e) & \textcd{type}~\mathtt{T}~\textcd{=}~\tau\\
                          && &&~~~~~~\textcd{syntax = }e\\
               &&|&\mathbf{tyaptsm}(\T,s,body,e) & \textcd{type}~\mathtt{T}~\textcd{=}~\mathit{s~dform}\\
%      \text{Object Fields}\\
%      \omega~::=~\emptyset                      \\
