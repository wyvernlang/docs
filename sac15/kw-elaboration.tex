...
\begin{center}
\AXC{$\Theta_0 \subset\Theta$ ~~~~ $\vdash_{\Theta}^{\Delta} \theta'\sim(\Theta';\Delta')$ ~~~~ $\nexists~k.(k[\mathbf{ty}(\_,\_)]\in\Delta\Delta')$ ~~~~ $\vdash_{\Theta\Theta'}^{\Delta\Delta'}e\rightsquigarrow i\Leftarrow \mathbf{named}[TypeParser]$ ~~~~ $\vdash_{\Theta\Theta'}\tau$}
...
\AXC{
	...
	{$\emptyset\vdash_{\Theta\Theta'}^{\Delta\Delta'} e_{k}\rightsquigarrow i_{k}\Leftarrow \mathbf{named}[ExpParser]$}
...
\AXC{
	\stackanchor
	{$\Theta_0\subset \Theta$ ~~~~ $\myvdash \theta'\sim(\Theta';\Delta')$ ~~~~ $\nexists~k.(k[\mathbf{ana}(\_)]\in \Delta\Delta'\lor k[\mathbf{syn}(\_,\_)]\in\Delta\Delta')$}
	{$\vdash_{\Theta\Theta'} \tau$ ~~~~ $\emptyset\vdash_{\Theta\Theta'}^{\Delta\Delta'} e_{k}\rightsquigarrow i_{k}\Leftarrow \mathbf{named}[ExpParser]$}
