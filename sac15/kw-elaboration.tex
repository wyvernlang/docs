...
\noLine
\UIC{$\vdash_{\Theta\Theta',T[?,?,?]}\tau$ ~~~~ $\emptyset\vdash_{\Theta\Theta',T[\tau,?,?]}e\rightsquigarrow i_m\Rightarrow\tau_m$ ~~~~ $\vdash^{\Delta}_{\Theta\Theta',T[\tau,i_m:\tau_m,?]}\kappa\rightsquigarrow\dot\kappa$}\RightLabel{~ (ctx-explicit-type)}
...
\UIC{$\myvdash \theta';T[\mathbf{tykey},k,body,e_m,\kappa] \sim \Theta',T[\tau,i''_m:\tau'_m,\dot{\kappa}\dot{\kappa}']$}
\DP
\end{center}
