...
\AXC{
	\stackanchor
	{$\Theta_0\subset \Theta$ ~~~~ $\myvdash \theta'\rightsquigarrow\Theta'$ ~~~~ $\nexists~k.(k[\mathbf{ana},\_]\in \Theta\Theta'\lor k[\mathbf{syn}(\_),\_]\in\Theta\Theta')$}
	{$\emptyset\vdash_{\Theta\Theta'} e_{k}\rightsquigarrow i_{k}\Leftarrow \mathbf{named}[ExpKw]$}
}\RightLabel{(expkw-wk)}
\UIC{$\myvdash\theta';k[\mathbf{ana},e_k]\rightsquigarrow \Theta';k[\mathbf{ana},i_k]$}
\DP
\end{center}
...
\UIC{$\myvdash\theta';k[\mathbf{syn}(\tau),e_k]\rightsquigarrow \Theta';k[\mathbf{syn}(\tau),i_k]$}
\DP
\end{center}
