...
\noLine
\UIC{$\vdash_{\Theta\Theta',T[?,?,?]}\tau$ ~~~~ $\emptyset\vdash_{\Theta\Theta',T[\tau,?,?]}e\rightsquigarrow i_m\Rightarrow\tau_m$ ~~~~ $\emptyset\vdash^{\Delta}_{\Theta\Theta',T[\tau,i_m:\tau_m,?]}\kappa\rightsquigarrow\dot\kappa$}\RightLabel{~ (ctx-explicit-type)}
...
\noLine
\UIC{$\texttt{parsestream}(body)=i_{ps}$ ~~~~ $\mathbf{iap}(\mathbf{iprj}[parse](\mathbf{iprj}[parser](i_k));i_{ps})\Downarrow(i_{type},i_{m},i_{kw})$} 
\noLine
\UIC{$i_{type}\uparrow\tau$ ~~~~ $\vdash_{\Theta\Theta',T[?,?,?]}^{\Delta}\tau$ ~~~~ $\emptyset\vdash_{\Theta\Theta',T[\tau,?,?]}e_w\Rightarrow i_m:\tau_m\rightarrow\tau'_m$ ~~~~ $\mathbf{iap}(i_w;i_m)\Downarrow i'_m$}
\noLine
\UIC{$\vdash_{\Theta\Theta',T[\tau,?,?]}i_m\Leftarrow\tau'_m$ ~~~~ $i_{kw}\uparrow\dot{\kappa}$ ~~~~ $\emptyset\vdash^{\Delta}_{\Theta\Theta',T[\tau,i'_m:\tau'_m,?]}\kappa\rightsquigarrow\dot{\kappa}'$ ~~~~ $dom(\dot{\kappa})\cap dom(\dot{\kappa}')=\emptyset$}\RightLabel{~ (ctx-typekw-type)}
\UIC{$\myvdash \theta';T[\mathbf{tykey},k,body,e_w,\kappa] \sim \Theta',T[\tau,i'_m:\tau'_m,\dot{\kappa}\dot{\kappa}']$}
\DP
\end{center}
