...
\AXC{
	%\stackanchor
	$d\sim(\Psi;\Theta)$ ~~~~ $s \notin \text{dom}(\Psi)$ ~~~~ $\emptyset\vdash_{\Theta_0\Theta} \tau::\star$ ~~~~ $\emptyset;\emptyset\vdash_{\Theta_0\Theta}^{\Psi} e_{tsm}\rightsquigarrow i_{tsm}\Leftarrow Parser(Exp)$
}\RightLabel{(D-syntsm)}
...
\star$ ~~~~ $\emptyset; \emptyset \vdash_{\Theta_0\Theta}^{\Psi}e_{tsm}\rightsquigarrow i_{tsm}\Leftarrow Parser(Type \times \tau_{md})$}
\RightLabel{(D-tytsm)}
...
\UIC{$d;\mathbf{tydecl}(T,\tau,e) \sim (\Psi; \Theta,T[\tau(T) :: \kappa,i_{md}:\tau_{md}])$}
\DP
\end{center}
...
\AXC{
	...
	{$\emptyset;\emptyset\vdash_{\Theta_0\Theta,T[\tau(T) :: \kappa,-]}^{\Psi}e_{mdx}\rightsquigarrow i_{mdx} \Rightarrow \mathbf{arrow}[\tau_{md}, \tau'_{md}]$ ~~~~ $i_{mdx}(i_{md})\Downarrow i'_{md}$}
}
\RightLabel{(D-aptsm)}
\UIC{$d;\mathbf{tyaptsm}(T,s,body,e_{mdx}) \sim (\Psi; \Theta,T[\tau(T) :: \kappa,i'_{md}:\tau'_{md}])$}
\DP
\end{center}
