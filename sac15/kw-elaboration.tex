...
\begin{center}
\AXC{$\Theta_0 \subset\Theta$ ~~~~ $\vdash_\Theta \theta'\sim\Theta'$ ~~~~ $k\notin dom(\Theta\Theta')$ ~~~~ $\vdash_{\Theta\Theta'}e\rightsquigarrow i\Leftarrow \mathbf{named}[TypeKw]$ ~~~~ $\vdash_{\Theta\Theta'}\tau$}
\RightLabel{(tykw-c)}
...
\begin{center}
\AXC{$\Theta_0\subset \Theta$ ~~~~ $\myvdash \theta'\rightsquigarrow\Theta'$ ~~~~ $k\notin dom(\Theta\Theta')$ ~~~~ $\vdash_{\Theta} \tau$ ~~~~ $\emptyset\vdash_{\Theta\Theta'} e_{k}\rightsquigarrow i_{k}\Leftarrow \mathbf{named}[ExpKw]$}	\RightLabel{(expkw-bk)}
