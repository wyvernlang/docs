...
$$\AXC{$\vdash_{\Theta_{0}}^{\emptyset}\theta\sim(\Delta;\Theta)$ ~~~~ $\emptyset\vdash_{\Theta_0\Theta}^{\Delta}e\rightsquigarrow i\Rightarrow \tau$}      \RightLabel{(program)}
\UIC{$\theta;e\sim(\Theta;\Delta)\rightsquigarrow i:\tau$}
\DP$$

The syntax for the contexts used in these judgements is shown in Figure \ref{typechecking-environment}. The rules for the judgements appearing as premises here are shown in Figures \ref{typechecking-elaboration} and \ref{expkw-kwstatics} and will be described in Sections \ref{declarations} and \ref{elaboration}, respectively.

\subsection{Declarations}\label{declarations}
The program compilation judgement begins by constructing contexts $\Delta$ and $\Theta$ from the TSM declarations and named types, respectively, in $\theta$. The TSM context begins empty, written $\emptyset$, but the named type context begins with the prelude, written $\Theta_0$. 
...
\begin{figure}[t]
  $\begin{array}{ll}
      ...
      \tabularspace\theta; T[\mathbf{explicit}(\tau, e)]  & \textcd{type}~T~\textcd{=}~\tau\\
                                                            & ~~~\textcd{metadata = }e\\
      \tabularspace\theta; T[\mathbf{tsmap}(k, body, e)]   & \textcd{type}~T~\textcd{=}~k~dform\\
                                                            & ~~~\textcd{metadata = }e\\
      \textsf{Types}\\
      \tau~::=~\mathbf{named}[T]              & T\\
...
%      \tabularspace C[\tau];\chi                   & C~\textcd{of}~\tau\\
      ...
      \tabularspace\mathbf{etsmap}[k,body]       & k~dform\\
