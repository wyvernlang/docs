...
\la{bnd}$\Gamma_1\vdash \sigma_l \sigbndl 1 \dashv \Theta$ \` By lem. 10 on $\Gamma \ctxmapto \Omega$ and \ref{initial2}\\
...
\>\>\la{abnd}$\Gamma\vdash [\Omega]\alphahat \sigbndr \pair{\rho}{\tau_u} \dashv\_$ \` Lem. 10 on $\Gamma \ctxmapto \Omega$\\
...
\la{l101} $\Gamma_2 \ctxmapto \Omega_l$ 										\` Lem. 10 on $\Gamma \ctxmapto\Omega$\\
...
\la{fn} $\Theta \vdash 1 \sigbndr \pair{\rho}{\top} \dashv\Theta_u''$				\` By Lem. 26 on \ref{bnd}, \ref{unit}, and by lemma 10 on $\Gamma\ctxmapto\Omega$\\
\la{finally} $\Theta_u \vdash 1 \sigbndr \pair{\rho}{1} \dashv \Theta_u''$ 			\` By Lem. 12 on \ref{unit} and \ref{fn}\\
\> $\Delta \ctxmapto \Omega$ \` By Lemma 11 on \ref{finally}, \ref{fn}\\
...
\>\>$\Gamma' \vdash \top \sigbndl \arrow{\tau_1}{\tau_2} \dashv \Omega$ \` By lem. 17 on \ref{taubnd}.\\
\>\>Contradiction\\
\>\la{th3mt}$\Theta_3 \ctxmapto \Omega'$ \` By \ref{gls} and \ref{th2pmt}\\
\la{2cont3}$\Theta_2 \contracts \Theta_3$ \` By Lem. 33\\
...
\>\la{bndext1}$\Theta_3 \vdash \tau_1' \st \alphahat_1 \dashv \Theta_3'$ \` By lem. 35 on \ref{ct3} and \ref{st1}\\
\>\la{th3pmt}$\Theta_3' \ctxmapto \Omega'$ \` By lemma 40 on \ref{th3mt} and \ref{bndext1}\\
\>\la{ct4}$\Theta_1'' \contracts \Theta_3'$ \` By lem. 34 on \ref{ct2}, \ref{2cont3}\\
...
\la{om1}$\Theta_4' \vdash \pair{\rho_l}{c_l} \sigbndl \arrow{\tau_1}{\tau_2} \dashv \_$ \` By lem. 10 on \ref{omm}\\
\la{breaklsig}$\Theta_4' \vdash \pair{\rho_l}{\bot} \sigbndl \arrow{\tau_1}{\tau_2} \dashv \Theta''$ \` By lem. 47 on \ref{om1}, for some $\Theta''$\\
...
\la{bndl}$\Theta_4' \vdash \pair{\rho_l}{\arrow{\alphahat_1}{\alphahat_2}} \sigbndl \arrow{\tau_1}{\tau_2} \dashv \Theta$\` By lem. 12 on \ref{arlst} and \ref{breaklsig}\\
\>WTS: $\Theta_4' \vdash \arrow{\tau_1}{\tau_2} \sigbndr \pair{\rho_t}{\arrow{\alphahat_1}{\alphahat_2}} \dashv \Theta'$ and $\Theta' \ctxmapto \Omega'$\\
\la{om2}$\Theta_4' \vdash \arrow{\tau_1}{\tau_2} \sigbndr \pair{\rho_t}{c_t} \dashv \_$ \` By lem. 10 on \ref{omm}\\
\la{breakrsig}$\Theta_4' \vdash \arrow{\tau_1}{\tau_2} \sigbndl \pair{\rho_t}{\top} \dashv \Theta''$ \` By lem. 48 on \ref{om2}, for some $\Theta''$\\
...
\la{bndr} $\Theta_4' \vdash \arrow{\tau_1}{\tau_2} \sigbndr \pair{\rho_t}{\arrow{\alphahat_1}{\alphahat_2}} \dashv \Theta'$\` By lem. 12 on \ref{arrst} and \ref{breakrsig}\\
...
Proceed by mutual structural induction on $B$.
\begin{tabbing}
Case $B = 1$\resla\\
