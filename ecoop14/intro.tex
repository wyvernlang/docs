...
\label{f-delims}
\end{figure}
Our first TSL appears on the right-hand side of the variable binding on line 1. The variable \li{imageBase} is annotated with its type, \li{URL}. This is a named {object type} declaring several fields representing the components of a URL: its protocol, domain name, port, path and so on (not shown). We could have created a value of type \li{URL} using general-purpose notation (using the keyword \li{new}, an expression form that \emph{forward references} an indented block of field and method definitions beginning on the line after it appears):
\begin{figure}[h]
\vspace{-22px}
...
This is tedious. By associating a TSL with the \li{URL} type (we will show how later), we can instead introduce precisely this value using conventional notation for URLs by placing it in the \emph{body} of a \emph{generic literal}, \li{<SURLimages.example.comEURL>}. Any other delimited form in Fig. \ref{f-delims} can equivalently be used when the constraints shown can be obeyed. The type annotation on \li{imageBase} (or equivalently, directly after the literal) implies that this literal's \emph{expected type} is \li{URL}, so the {body} of the literal (the characters between the angle brackets, in blue) will be governed by the \li{URL} TSL during the typechecking phase. This TSL will parse the body ({at compile-time}) to produce a Wyvern abstract syntax tree (AST) that explicitly instantiates a new object of type \li{URL} using general-purpose notation as if the above had been written directly.
...
SGRM      start <- '>body'= attributes start>
        EGRMfn (attrs, child) => `SQTBodyElement((%EQTattrsSQT%, %EQTchildSQT%))EQT`
