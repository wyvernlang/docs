...
The remainder of the paper is organized as a language design for TSL Wyvern:
\begin{enumerate}
...
\item \textbf{Hygienic Elaboration}: Elaboration of literals must not cause the inadvertent capture or shadowing of variables in the context where the literal appears. It must, however, remain possible to do so in portions of the parse stream designated during parsing as spliced expressions. The language cannot know \emph{a priori} where these spliced portions will be. We show a clean type-theoretic formulation for this notion of hygiene.
\end{enumerate}
