...
$\\[2ex]
%\vspace{-12px}
\caption{Abstract Syntax of TSL Wyvern external terms ($e$), internal terms ($i$) and translational internal terms ($\ih$). Metavariable $T$ ranges over type names, $\ell$ over object member (field and method) labels, $C$ over case labels, $x$ over variables and $body$ over literal bodies. Tuples can be introduced as derived forms, so we omit them from the abstract syntax: $(i_1, i_2) := \keyw{inew}\,\{\keyw{ival}[\ell_1](i_1); \keyw{ival}[\ell_2](i_2)\}$.
}
...
\end{figure}
\begin{figure}[t]
$\fbox{$i \xmapsto[\Sigma]{} i$}$~~~~~~$\cdots$~~~~$\begin{array}{c}
\infer[\textit{D1}]
