...
\item The \li|parse| method is called with this parse stream. If it evaluates to a reified elaboration, $i_{ast}$ (of type \li{Exp}) and a remaining parse stream, $i'_{ps}$, then parsing was successful. Note that we use shorthand for pairs in the rule for concision, and the relation $i \Downarrow i'$ defines evaluation to a value (the maximal transitive closure of the small-step evaluation relation in Fig. \ref{fig:dynsemantics}).
