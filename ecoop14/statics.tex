...
\infer[\textit{RT-objtype}]
	  ...
	  {\Delta \vdash \omega & \Delta; \Gamma \vdash e \Rightarrow \tau \leadsto \hat{e} & \Delta, t:\{\omega,e_0:\tau\}; \Gamma \vdash \rho :\tau'\leadsto \hat{\rho} }
	   \\[3ex] 
...
\infer[\textit{RT-casetype}]
	  ...
	  {\Delta \vdash \chi & \Delta; \Gamma \vdash e \Rightarrow \tau \leadsto \hat{e} & \Delta, t:\{\chi,e_0:\tau\}; \Gamma \vdash \rho :\tau'\leadsto \hat{\rho} }
	   \\[3ex] 


\infer[\textit{RT-e}]
	{\Delta; \Gamma \vdash  e;\rho:\tau'\leadsto \hat{e};\hat{\rho}} 
	{\Delta \vdash \omega & \Delta; \Gamma \vdash e \Rightarrow \tau \leadsto \hat{e} & \Delta; \Gamma \vdash \rho:\tau'\leadsto \hat{\rho} }\\[3ex]
...
\infer[\textit{T-new}]
	{\Delta; \Gamma \vdash \keyw{new}\ \{ d \} \Leftarrow  t }
	{ t:\{\omega,e_0:\tau\} \in \Delta & \Delta;\Gamma \vdash d \Leftarrow \omega & t\neq TokenStream} \\[3ex]

\infer[\textit{T-new-hat}]
	{\Delta; \Gamma \vdash \keyw{new}\ \{ d \} \Leftarrow  t }
	{ t:\{\omega,e_0:\tau\} \in \Delta & \Delta;\Gamma \vdash d \Leftarrow \omega} \\[3ex]
...
\infer[\textit{T-fromTS}]
	  {\Delta; \Gamma' \vdash fromTS[\Gamma](e_1,e_2) \Leftarrow \tau \leadsto \hat{e} }
	  {\renewcommand{\arraystretch}{1}
	    \begin{array}{r}
	    \Delta;\Gamma' \vdash e_1:TokenStream ~~~~~~ \Delta;\Gamma' \vdash e_2:Token\\
            parse(e_1,e_2)=e ~~~~~~\Delta; \Gamma', \Gamma \vdash e \Leftarrow \tau \leadsto \hat{e}
            \end{array}
       } \\[3ex]  
...
\infer[\textit{T-literal}]
	  ...
	  {\renewcommand{\arraystretch}{1}
	    \begin{array}{r}
	    \Delta;\Gamma \vdash t.\keyw{metaobject}.parser \Leftarrow Parser \leadsto \hat{e}_p ~~~~~ \text{TokenStream of~} \lfloor literal \rfloor\ \text{is}\ \hat{e}_{ts}\\
            \hat{e}_p.parse(\hat{e}_{ts}) \Downarrow_{\Delta} Exp.C(\hat{e}') ~~~~~  e \triangleleft Exp.C(\hat{e}')~~~~~ \Delta;\Gamma\vdash e\Leftarrow t \leadsto \hat{e}
            \end{array}
       } \\[3ex]   
\end{array}
\]
